...
# ADAPTER_REGISTRY_SHA256=4b2b9bc9ae716aff0bd43ce251dcbe70416f916045e15131cfda4f9c29c0716c
...
SLP_RUNTIME_PATHS_PY
  ...
  if (( _slp_rc == 0 && _slp_attempt < 3 )) && [[ $_slp_obs == ERROR$'\t'* ]]; then
    case ${_slp_obs#ERROR$'\t'} in
      proc-population:process-disappeared|proc-stat:initial-starttime-changed|proc-stat:initial-endtime-changed|proc-exe:read-failed|proc-status:read-failed|proc-maps:read-failed|proc-stat:excluded-classification-vanished|proc-stat:excluded-classification-changed|pid-population:mid-snapshot-changed|proc-counter:mid-snapshot-changed|proc-stat:recheck-starttime-changed|proc-exe:recheck-missing|proc-exe:recheck-changed|proc-maps:recheck-changed|proc-stat:recheck-endtime-changed|proc-stat:excluded-recheck-changed|proc-exe:excluded-reappeared|pid-population:final-snapshot-changed|proc-counter:final-snapshot-changed) command /usr/bin/sleep 1; continue ;;
    esac
  fi
  break
  done
