...
# ADAPTER_REGISTRY_SHA256=06f29b1beb9263566b28a73fdb17c67caf478cff0949db78fc0c017bce71597b
...
slp_check_FSTEC_LINUX_2022_2_1_2_SSH_ROOT_LOGIN() {
  ...
  if [[ -L "$_slp_sshd" ]]; then
    _slp_real=$(/usr/bin/readlink -f -- "$_slp_sshd" 2>/dev/null) || _slp_real=
  ...
  _slp_parse_sshd_file() {
    ...
    _slp_pr=$(/usr/bin/readlink -f -- "$_slp_pf" 2>/dev/null) || { _slp_parser_error=1; return 0; }
        ...
        for _slp_pp in "${_slp_args[@]}"; do
          ...
          if [[ "$_slp_px" == *'*'* || "$_slp_px" == *'?'* || "$_slp_px" == *'['* ]]; then
            ...
            if [[ -e "$_slp_prefix" ]]; then
              ...
              _slp_nl=$(/usr/bin/find "$_slp_prefix" -name $'*\n*' -print -quit 2>/dev/null)
              _slp_grc=$?
              (( _slp_grc == 0 )) || { _slp_parser_error=1; break 2; }
              [[ -z "$_slp_nl" ]] || { _slp_parser_error=1; break 2; }
            fi
            _slp_glob_text=$( ( set -o pipefail; builtin compgen -G "$_slp_px" | LC_ALL=C "$_slp_sort" ) )
            _slp_grc=$?
            if (( _slp_grc == 0 )); then
  ...
  "$_slp_sshd" -t -f "$_slp_cfg" >/dev/null 2>&1
  _slp_rc=$?
  if (( _slp_rc != 0 )); then
  printf "%s\t%s\t%s\t%s\t%s\n" 'SLP-CHECK-V1' 'FSTEC-LINUX-2022-2.1.2-SSH-ROOT-LOGIN' "ERROR" "-" "ERROR"
    return 0
  fi
  _slp_effective=$("$_slp_sshd" -T -C user=root,host=localhost,addr=127.0.0.1 -f "$_slp_cfg" 2>/dev/null)
  _slp_rc=$?
  if (( _slp_rc != 0 )); then
  printf "%s\t%s\t%s\t%s\t%s\n" 'SLP-CHECK-V1' 'FSTEC-LINUX-2022-2.1.2-SSH-ROOT-LOGIN' "ERROR" "-" "ERROR"
    return 0
