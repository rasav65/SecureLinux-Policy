...
# ADAPTER_REGISTRY_SHA256=a9c8b36516634c78f8674d782c8e3211623fbe3c5a3f227289a2ec1619b5bdfa
...
SLP_RUNTIME_PATHS_PY
  ...
  if (( _slp_rc == 0 && _slp_attempt < 3 )) && [[ $_slp_obs == ERROR$'\t'* ]]; then
    case ${_slp_obs#ERROR$'\t'} in
      proc-population:process-disappeared|proc-stat:initial-starttime-changed|proc-stat:initial-endtime-changed|proc-exe:read-failed|proc-status:read-failed|proc-maps:read-failed|proc-stat:excluded-classification-vanished|proc-stat:excluded-classification-changed|pid-population:mid-snapshot-changed|proc-counter:mid-snapshot-changed|proc-stat:recheck-starttime-changed|proc-exe:recheck-missing|proc-maps:recheck-changed|proc-stat:recheck-endtime-changed|proc-stat:excluded-recheck-changed|proc-exe:excluded-reappeared|pid-population:final-snapshot-changed|proc-counter:final-snapshot-changed) command /usr/bin/sleep 1; continue ;;
    esac
  fi
  break
  done
