...
assert "Текущая активная стадия: **mandatory real-jsonschema release gate**." in readme
assert "The current authorized engineering step is `mandatory real-jsonschema release gate`." in readme
...
assert "NEXT: mandatory real-jsonschema release gate." in roadmap
...
assert by_id["TYPE_BOOLEAN_CONTRACT_CLEANUP"] == "CLOSED_BY_THIS_CHANGE"
assert by_id["MANDATORY_REAL_JSONSCHEMA_RELEASE_GATE"] == "NEXT"
assert pmap.count(":::current") == 1

print("CURRENT_STATUS_CONSISTENCY=PASS next=MANDATORY_REAL_JSONSCHEMA_RELEASE_GATE")
