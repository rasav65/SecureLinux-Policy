...
`test_schema_runtime_parity.py` — regression для B-R1-01 и B-R2-01:
...
- семантика pattern: для каждого anchored pattern `re.fullmatch` (runtime) и `re.search` (JSON Schema) должны принимать одно и то же множество строк;
