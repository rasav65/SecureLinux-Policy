...
- `sshd-config-option-v1`: исправление по аудиту `74aaa70..dcc61b7` (B-01..B-03). Строк source
  ...
  из корня тестового дерева с возвратом внутрь принималось; самопроверка `assert_state` не
  проверяла отсутствие ожидаемого временного файла. Стало: завершающий «/» требует каталог;
  «..» выше корня тестового дерева — отказ; добавлены регрессии. В HANDOFF — граница
  проверки ключа администратора (решение пользователя 26.09.2026): ACL, SELinux/AppArmor,
  NSS не из файлов, `AuthorizedKeysCommand` вне объёма. Меняет продукт: ВМ-прогона нет.

- `sshd-config-option-v1`: исправление по аудиту `943483a..74aaa70` (B-01, B-02). Строк source
  index не закрывает. Было: символические ссылки разрешались только для домашнего каталога —
  каталоги цели ссылки `~/.ssh` (например `/srv/private`) на проход не проверялись; тесты
  FAILED_COMPENSATION сверяли временные файлы только по имени и типу. Стало: путь ключа
  разбирается по компонентам с каждым переходом по ссылке (абсолютной и относительной, цепочки,
  петля — ключа нет), бит прохода проверяется на каждом пройденном каталоге; StrictModes —
  от фактического каталога ключа вверх до домашнего каталога или «/», как `auth_secure_path`
  sshd; тесты сравнивают всё дерево `/etc/ssh` с ожидаемым, включая путь, байты, режим и
  UID/GID временных файлов. Меняет продукт: ВМ-прогона нет.

- `sshd-config-option-v1`: исправление по аудиту `e11ad4c..943483a` (B-01, B-02). Строк source
  index не закрывает. Было: ключ администратора засчитывался без проверки, что администратор
  прочитает файл и пройдёт по каталогам (root читает любой файл); после FAILED_COMPENSATION
  тесты проверяли только два файла. Стало: доступ по битам владельца, группы (основная и
  дополнительные из /etc/group) и прочих от «/» до файла ключа по фактическому пути (ссылки
  разрешаются); тесты сверяют всё дерево
  `/etc/ssh` (тип, байты, режим, UID/GID, временные файлы), а каждый FAILED_COMPENSATION
  обязан пройти эту сверку. В HANDOFF записаны решения пользователя 26.09.2026 (парольная
  политика, 11.2, 2.3.x, 8.4, auditd). Меняет продукт: ВМ-прогона нет.

- `sshd-config-option-v1`: B-15/B-16 аудита `5e19919..4d3aadc` и независимая предпроверка.
  Аргументы разбираются только у `Match`, `Include` и ключа контроля и так же, как
  `_slp_split_args` CHECK (`Include /etc/ssh/a#b.conf` — один аргумент; обратная косая
  черта — обычный символ, как в CHECK); равенство проверяется дифференциальным тестом против
  bash-функции CHECK, разбор файла — сравнением с CHECK на 400 конфигурациях. Ссылка вместо
  основного `sshd_config` — блок «решение администратора». Ключ администратора засчитывается
  при StrictModes только если `~`, `~/.ssh` и файл принадлежат пользователю или root без
  записи g/o. Запрет входа root — только если хотя бы один администратор войдёт сам (пароль
  или пригодный ключ); AllowUsers/DenyUsers/AllowGroups/DenyGroups и AuthenticationMethods не
  `any` — администратор не засчитывается (решение администратора). Байты не UTF-8
  переносятся без изменений, как принимает CHECK. Оставшийся временный файл учитывается как
  изменение (`mutation_performed`).
  Тесты: общая проверка дерева `/etc/ssh` (байты, тип, полный режим, `.slp-tmp`) в каждом
  вызове, явные режимы FIFO и каталогов ключей. Меняет продукт: ВМ-прогона нет.
- Главная страница, B-01/B-02 аудита `4d3aadc..e11ad4c`: название `fstec-configuration-2026`
  в `docs/policy-layers.md` приведено к названию документа; тест сверяет названия таблицы с
  policy-layers и долю APPLY — предложением целиком, число из `apply.supported` YAML.
- Предупреждение Ubuntu Desktop перед APPLY — рамка одной ширины вместо строк «!!!»;
  строки CHECK/APPLY идут по документам в порядке года (fstec-linux-2022 выше
  fstec-configuration-2026), внутри документа — порядок манифеста (решения пользователя
  26.09.2026). Строк source index не закрывает.

- Главная страница: раздел «Прогресс» — таблица строк индекса по документам ФСТЭК
  (закрыто контролями, диспозицией, открыто), итог и доля контролей с APPLY. Блок
  машинный: `tools/render-current-docs.py` строит его из SOURCE-INDEX и манифестов,
  `tests/roadmap-v1/test_current_status.py` пересчитывает независимо. Исправлено в README:
  подзаголовок, значок APPLY и список возможностей не упоминали APPLY параметров загрузки,
  `su` и SSH; пропущенная запятая в разделе «Применение изменений». Строк source index
  не закрывает, байты продукта не меняет.

- Таблицы CHECK и APPLY (решение пользователя 26.09.2026). Строк source index не закрывает.
  Было: пунктира над заголовком не было; `source` 18–24 символа, «fstec-configuration-2026
  §9.1» переносился. Стало: пунктир над заголовком основной таблицы и таблицы блоков; от
  100 колонок `source` — 29 символов, `control` уже на 3 (25/29/33), `current` уже на 2
  (100 колонок — 12/14, 116 — 14/24, 139 — 31/26, 160 — 33/45 для current/required;
  решение 19.09.2026 заменено). Описание вывода в product/README исправлено: колонки
  `RESULT`/`CONTROL`/`VALUE / DETAILS` не существуют.
- `sshd-config-option-v1`, B-15 аудита `6a0634c..5e19919`: регрессии проверяют байты,
  полный режим и оставшиеся `.slp-tmp`; добавлены SGID, SUID+SGID, сохранение специальных
  битов при откате, ошибка `close` при первоначальном чтении. Только тесты (56 случаев).

- `sshd-config-option-v1`: исправление по аудиту `f75181a..6a0634c` (B-12…B-14). Строк
  source index не закрывает. Было: строки делились `splitlines()` (\v в комментарии давал
  директиву и ложное `ALREADY_COMPLIANT`); ошибки `close` обходили очистку и компенсацию;
  `fchmod` до `fchown` терял SUID/SGID. Стало: деление только по LF и пробельные символы как
  `[[:space:]]` CHECK; ошибки `close` учитываются (временный файл удаляется или передаётся
  в компенсацию, сверка — `FAILED_COMPENSATION` с продолжением возврата); владелец, затем
  полный режим. Дополнительно: `/etc/group` и `/etc/passwd` читаются без ожидания (FIFO —
  отказ). Тест — 53 случая. Меняет продукт: ВМ-прогона нет.

- `sshd-config-option-v1`: исправление по аудиту `f75181a..001f9c2` (B-11). Строк source
  index не закрывает. Было: если после ошибки записи или `fsync` временный файл не удалялся,
  компенсация о нём не знала и возвращала `FAILED_NOT_COMMITTED`. Стало: такой временный
  файл передаётся в компенсацию (прямая подготовка и восстановление), ошибка его удаления
  даёт `FAILED_COMPENSATION`, возврат остальных файлов продолжается. Тест — 46 случаев.
  Меняет продукт: ВМ-прогона нет.

- `sshd-config-option-v1`: исправление по повторному аудиту `f75181a..4b0bed3` (B-08…B-10)
  и по B-04 первого прогона аудита `f75181a..b1506bd`. Строк source index не закрывает.
  Было: ошибка `os.read` обходила компенсацию; ошибка удаления временного файла
  останавливала возврат файлов; при ошибке восстановительной замены оставался `.slp-tmp`;
  настройки ключей брались из `sshd -T` для root. Стало: ошибки чтения — контролируемый
  отказ, при сверке восстановления — `FAILED_COMPENSATION` с продолжением возврата;
  ошибки очистки накапливаются; временный файл восстановления удаляется; `PubkeyAuthentication`
  и `AuthorizedKeysFile` берутся из `sshd -T` для соединения каждого администратора (`Match User`).
  Тест — 43 случая. Меняет продукт: ВМ-прогона нет.

- `sshd-config-option-v1`: исправление по аудиту `f75181a..b1506bd` (B-02…B-07; B-01 —
  расхождение версии задания аудитору со схемой 1–4, код не меняется). Строк source index
  не закрывает. Было: `sshd -t` только после замены файлов; FIFO блокировал открытие;
  доверие основного файла не проверялось при соответствии и при правке только включаемого;
  запись без проверки полноты, восстановленные байты не сверялись; каталог-ссылка в
  префиксе `Include` допускался. Стало: каждый подготовленный файл проверяется `sshd -t` до
  замены; открытие `O_NONBLOCK` и тип по дескриптору; основной файл — обычный файл root без
  записи g/o до признания соответствия; запись до последнего байта, восстановленные байты
  сверяются (иначе `FAILED_COMPENSATION`); правила `Include` как у CHECK. Тест — 38 случаев.
  Меняет продукт: ВМ-прогона нет.
...
- Генератор source-блока: пункт 9.4 fstec-configuration-2026, напечатанный как «8.4»
  ...
  закрывает, байты продукта не меняет. Было: отказ `SOURCE_NUMBERING_MISMATCH`,
  подпункт 9.3 захватывал текст 8.4. Стало: 9.3 (`SRC-0090`) — `EXACT`, 8.4 извлекается
  (отказ только из-за номеров страниц внутри); покрытие `EXACT` 119, `REFUSED` 12,
  `UNSUPPORTED` 218. По B-01 аудита `c5d12f1..d1edcac`: отказ подпункта срабатывает на
  отдельное целое число любой длины (`SUBPOINT_INTEGER_TOKEN`), а не только 1–3 цифры.
...
- Step 7B: решения пользователя 25.09.2026 по fstec-configuration-2026 пп.9.1, 9.2.
  ...
  Для п.9.1 решено: три контроля с APPLY; контроли каждого документа — в своём каталоге;
  порядок работ — в очереди `docs/HANDOFF.md`.
  Было: закрыто 333 из 349, диспозицией 293, открыто 16. Стало: закрыто 334,
  диспозицией 294, открыто 15. Байты продукта не меняются.

- Step 7B: read-only разведка 7 сред 25.09.2026 (архив
  `slp-vm-probe-supported7-v1-states1-7-20260925-211959.tar.gz`, SHA `28c7202ad6fbf9943978fc1acb678da90787eb026191f1bf012f243c4ab13a98`) и диспозиция
  `SRC-0066` (fstec-configuration-2026 п.3.2) как `out-of-scope`: пакеты samba и smbclient
  отсутствуют на всех 7 средах. Было: закрыто 332 из 349, диспозицией 292, открыто 17.
  Стало: закрыто 333, диспозицией 293, открыто 16; факты разведки — в очереди
  `docs/HANDOFF.md`. Байты продукта не меняются.

- Step 7B, исправление по аудиту `dc44a48..612b99c` (B-01, B-02). Было: `SRC-0096`
  (fstec-configuration-2026 п.10.5) закрыт как `organizational`, хотя строки 662–664
  источника задают правило auditd без входных данных администратора; basis `SRC-0042`
  захватывал начало п.3, basis `SRC-0048` обрывался на строке 83, а причина `SRC-0048`
  исключала Linux без основания в источнике. Стало: `SRC-0096` — снова `OPEN` (очередь
  HANDOFF), запись ledger снята; диапазоны 22–25 и 75–95; причина `SRC-0048` — настройка
  сертифицированных средств, включая операционные системы, по их документации. Закрыто
  332 из 349, диспозицией 292, открыто 17. Байты продукта не меняются.
...
- Действующий runner ВМ — `tools/vm-runner/slp-vm-apply-supported7-v17.sh`; v15 и v16
  ...
  запуск — копией из Загрузок, пункт очереди «Инфраструктура» без определения. Стало:
  HANDOFF описывает v17 (запуск из репозитория, явный `EXPECTED_CHECK_SHA256`); пункт
  закрыт: 7 сред — `product/SUPPORTED-PLATFORMS.tsv`, evidence — постоянный каталог,
  SHA архивов — этот журнал.

- Приёмка повтора наблюдения 2.3.2 по ВМ-прогону 25.09.2026: runner v17 на
  кандидате `dcf6ceaf6f349e13c2918c019f93c26eb96cece97495812e6db24adbdf3eef28`,
  архив `slp-vm-apply-supported7-v17-states1-7-20260925-175302.tar.gz` SHA
  `fee5246929e86f0ad753b9004c14972a0149cb8eaf09620ddfde34b5ee972276`, 81 файл.
  Строк source index не закрывает, байты продукта не меняет. Было: пункт 1 очереди
  `docs/HANDOFF.md` — прогон с ожиданием CHECK без `ERROR`. Стало: на 7 средах CHECK
  до и после APPLY без `ERROR`, 2.3.2 — PASS во всех 14 проверках; пункт снят.

- Ссылки на коммиты после переписывания истории 25.09.2026: из сообщений коммитов
  удалены строки соавторства (регламент v26 §37.A), содержимое деревьев не изменилось,
  хеши начиная с `0275908` сменились. Старые короткие хеши в `CHANGELOG.md`,
  `docs/HANDOFF.md` и комментариях `tests/product-v1/test_product_generator.py`
  заменены новыми (27 вхождений). Строк source index не закрывает, байты продукта не
  меняет.

- 2.3.2: `proc-maps:recheck-changed` исключён из `RETRY_REASONS` (B-01 аудита
  `863b754..f44c372`). Строк source index не закрывает. Было: запись карты
...
- 2.3.2: `proc-exe:read-failed`, `proc-status:read-failed`, `proc-maps:read-failed`
  исключены из `RETRY_REASONS` (B-02 аудита `2ff7cc2..863b754`). Строк source index не
...
- 2.3.2, исправление повтора наблюдения по аудиту `b900fc7..2ff7cc2` (B-01, B-02).
...
- Описание модуля `product-suid-sgid-applications-mode-apply-v1.py` (2.3.9). Строк
  ...
  ВМ-прогона кандидата `1836091` — REUSED: код не изменён.
...
- `supported_ops` в шести семантических контрактах CHECK: `home-directories-mode`
  ...
  `ADAPTER-REGISTRY.tsv`. Применимость ВМ-прогона кандидата `1836091` — REUSED:
  код адаптеров не изменён.

- Приёмка ВМ-прогона кандидата `1836091` (артефакт `d348b953…32be7`) после B-02;
...
- CHECK-адаптер `pam-wheel-access-v2` (2.2.1; B-02, аудит диапазона
  `6afb18e..41c3061`, B-01): итоговый `NOT_FOUND` выбирается по признакам
  доказанного ENOENT. Строк source index не закрывает. Было: после первых проверок
  условие `! -e pam || ! -e group` проверялось повторно, и ошибка stat при
  повторном наблюдении давала `NOT_FOUND` без ENOENT. Стало: признаки
  `_slp_pam_absent`/`_slp_group_absent` ставятся только после доказанного ENOENT;
  исчезновение или ошибка после первой проверки ведут к дальнейшим проверкам и
  `ERROR`. Тест `test_pam_wheel_not_found_only_from_proven_enoent` (до правки —
  FAIL). Применимость ВМ-прогона 25.09.2026 — REUSED по той же причине, что для
  предыдущей правки адаптера.
...
- CHECK-адаптер `file-mode-owner-v2` (2.3.1; B-02): отсутствие цели — только
  ...
  при доступном родителе давал `NOT_FOUND`. Стало: `NOT_FOUND` только при
  `stat -c %F` с текстом «No such file or directory» (LC_ALL=C задан в команде);
  иначе — `ERROR target:stat-failed`. Тест
  `test_stat_error_other_than_enoent_is_error` (до правки — FAIL); проверка
  подстановок допускает ровно одну `stat -c %F`. Применимость ВМ-прогона
  25.09.2026 — REUSED: ветка выполняется только при неудачном `stat -L`, на
  7 средах все три цели читаются (CHECK без `ERROR`).
...
- CHECK-адаптер `kernel-cmdline-v2` (2.4.3–2.4.7, 2.5.1, 2.5.3, 2.5.9; B-02):
  ...
  ENAMETOOLONG при доступном родителе давал `NOT_FOUND`. Стало: `stat -c %F`
  (семантика lstat) и буквальный текст «No such file or directory» дают
  `NOT_FOUND`; любая другая ошибка и висячая ссылка — `ERROR cmdline:read-failed`.
  Тесты: `test_kernel_cmdline_stat_error_other_than_enoent_is_error` (до правки —
  FAIL), `test_kernel_cmdline_dangling_symlink_is_error`; self-test адаптера
  допускает ровно одну подстановку `stat`. Применимость ВМ-прогона 25.09.2026 —
  REUSED: изменённая ветка выполняется только при недоступном `/proc/cmdline`, на
  7 средах он читается (CHECK без `ERROR`); итоговый ВМ-прогон — после всех
  адаптеров B-02.
...
- Документы после сверки очереди 25.09.2026 (решения по делегированию
  ...
  APPLY для kernel cmdline и G5, хотя `kernel-cmdline-grub-v1` (`3163834`) и
  `pam-wheel-su-v1` (`0b8cb6a`) реализованы; очередь `docs/HANDOFF.md` содержала
  неопределённый «B-02» и число «63» без основания. Стало: механизмы названы по
  именам без числа, текущий кандидат — по sidecar (как в `docs/compatibility.md`);
  в отложенном остались SRC-0008, G3 и инвентарь; `tests/roadmap-v1/test_project_map.py`
  требует отсутствия kernel cmdline и G5 в отложенном; очередь HANDOFF — с
  источниками, B-02 определён (v70 §7 п.4), 63/35 — роли OPEN-строк SOURCE-INDEX v4,
  2.3.1 и 2.6.6 закрыты ссылками.
...
- Инфраструктура шагов правки, решение человека 24.09.2026. Строк source index
  ...
  байтов и DEV/RELEASE; порядок работы и особенности рабочего ПК в репозитории
  не записаны. Стало: `tools/run-edit-step.sh КАТАЛОГ_ШАГА` выполняет эти
  проверки для каталога шага (`step.env`, `edit.py`, `expected.txt`,
  `commit-msg.txt`); `docs/HANDOFF.md` — порядок работы, особенности рабочего
  ПК (`umask 0002`, `/tmp` с `noexec`), известный дефект `refresh-pins`,
  ВМ-прогоны и очередь; строка в `docs/README.md`. Отдельного теста у
  `tools/run-edit-step.sh` нет; проверен shellcheck и прогоном шага на копии.

- Заголовок раздела блоков APPLY, решение человека 24.09.2026. Строк source
  index не закрывает, исходы и JSON-отчёт не меняет. Было: строка `blocks`,
  затем «Автоматическое изменение пропущено[. Требуется решение
  администратора].». Стало: одна строка, определяющая статус `block` основной
  таблицы, с числом блоков: «block — не применено автоматически, требуется
  решение администратора (N):»; без «требуется решение администратора», если в
  таблице есть блок без `operator_decision` (тогда `note` у каждого блока с
  решением, как раньше). Синхронизирован `docs/compatibility.md`. Тесты
  `test_product_generator.py`: ожидаемые строки заголовка и поиск раздела по
  префиксу `block — `. Трекнутый артефакт перегенерирован, `CHECK_SHA256`
  `021b964e6adcf1f6fbe980171355e9e6a69c1886a45e5f16e617a623af766443`.

- Вывод раздела `blocks` APPLY (генератор, dispatcher), решение человека
  24.09.2026. Строк source index не закрывает, исходы и JSON-отчёт не меняет.
  Было: каждый блок повторял «Автоматическое изменение пропущено. Требуется
  решение администратора», у параметров загрузки — ещё инструкцию
  `GRUB_CMDLINE_LINUX`/`update-grub`/перезагрузка и риск (у трёх iommu один и
  тот же). Стало: над таблицей один раз «Автоматическое изменение пропущено»,
  с «Требуется решение администратора», если решение нужно всем блокам (иначе
  строка `note` у каждого блока с `operator_decision`); строка `add:` с
  инструкцией — один раз при блоках параметров загрузки. Блок параметра
  загрузки — строка `add` с токеном и строка `risk`; одинаковый риск у блоков
  подряд — один раз после последнего. Код причины (`admin-decision:*`) остаётся
  в JSON-отчёте. Трекнутый артефакт перегенерирован, `CHECK_SHA256`
  `d840ede3b3887ef498c4baf30cd5daf1052e186342ac6bfcc287b87ce42bc8aa`.

  Тесты (`test_product_generator.py`): побайтный вывод блока 2.6.6 обновлён
  (`test_apply_blocks_precondition_conflict_output_exact`), блок параметра
  загрузки — строки `add`/`risk`, блок без `operator_decision` — без «требуется
  решение»; новый `test_apply_blocks_common_phrases_once`: один риск на три
  блока подряд и `note` у каждого блока с решением при блоке без него.
...
- `tools/refresh-pins.py`: откат при отказе `--write` (решение человека
  ...
  дерево частично изменённым; найдено 24.09.2026 — `--write` отказал на review
  baseline, а `tests/*/SHA256SUMS` и `tools/SHA256SUMS` уже были переписаны.
  Стало: перед `--write` запоминаются байты и режимы всех файлов, видимых Git;
  при любом отказе они восстанавливаются, созданные запуском файлы удаляются,
  выводится `REFRESH_PINS_ROLLBACK=<число файлов>`, код 2. Тест
  `test_refresh_pins.py`: изменённый документ без `--reviewed` — отказ, число
  восстановленных файлов больше 0, дерево побайтно и по режимам равно дереву
  до запуска.

- Формула `truth_sha256` гейта документации (решение человека 24.09.2026,
  п.2.7 разбора слабых мест). Строк source index не закрывает, байты продукта
  не меняет. Было: `truth_sha256` считался по файлам `TRUTH_INPUTS` целиком;
  правка любого адаптера меняла SHA-колонки `ADAPTER-REGISTRY.tsv`, и гейт
  требовал пересмотра всех 17 документов текущего состояния (так было во
  всех четырёх коммитах правок адаптеров 24.09.2026). Стало: каждый файл
  `TRUTH_INPUTS` берётся без колонок `sha256` и `*_sha256`
  (`ADAPTER-REGISTRY.tsv`, `APPLY-KIND-REGISTRY.tsv`,
  `APPLY-IMPLEMENTATION-REGISTRY.tsv`, `CONTROL-MANIFEST.tsv`,
  `SOURCE-INDEX.tsv`); строка другой ширины, чем заголовок, — ошибка.
  Пересмотр требуется при смене состава, а не пинов. Смена байтов control-yaml
  без смены колонок `key`/`expected` `CONTROL-MANIFEST.tsv` пересмотра больше не
  требует. Формула одна в `test_documentation_baseline.py` и
  `tools/refresh-pins.py`. Значение `truth_sha256` 17 строк review baseline
  пересчитано по новой формуле без смены данных: на HEAD `5054831` все 17
  документов совпадали с прежним значением. Тест
  `test_refresh_pins.py`: правка адаптера проходит `--write` без
  `--reviewed-truth`, `truth_sha256` не меняется; смена `reason` в
  `DISPOSITION-LEDGER.tsv` без `--reviewed-truth` — отказ, с ним — PASS.

- Инструмент `tools/refresh-pins.py` (решение человека 24.09.2026, п.2.7
  разбора слабых мест). Строк source index не закрывает, байты продукта не
  меняет. Было: после правки исходника пины обновлялись вручную по шагам
  (пины адаптера в `.json` и `ADAPTER-REGISTRY.tsv`, генератор и трекнутая
  пара, вложенные `SHA256SUMS` по `pin-closure`, review baseline, корневые
  manifests). Стало: `--write` выполняет эти шаги одной командой, `--check`
  ничего не пишет и возвращает 1 при устаревшем пине. Обрабатываются только
  носители, где есть путь, изменённый относительно HEAD; устаревший пин
  неизменённого пути и новый файл вне manifest — ошибка; строка review
  baseline изменённого документа — только с `--reviewed DOC`, `truth_sha256` —
  только с `--reviewed-truth`. Описание — `docs/root-manifest-policy.md`.
  Тест `tests/project-integrity-v1/test_refresh_pins.py` (на копии дерева):
  формула `truth_sha256` совпадает с гейтом документации; согласованное дерево
  — PASS; правка адаптера — STALE, после `--write` — PASS; документ без
  `--reviewed` — отказ; устаревший пин неизменённого пути и неучтённый файл —
  отказ. Пины этого коммита обновлены самим инструментом.
...
- CHECK-адаптер `sudoers-reviewed-policy` (2.2.2, SRC-0004), решение человека 24.09.2026. Строк
  ...
  Синхронизированы семантический контракт, пины адаптера в `.json` и
  `ADAPTER-REGISTRY.tsv`, `product/README.md`, `tests/product-v1/README.md`.
  Трекнутый артефакт перегенерирован, `CHECK_SHA256` `5fe8bb9a8925022bd68a3aeb91c00c3a78f0778f129f6e5af8c69baf9e6d3679`.

  Тесты (`SudoersReviewedPolicyAdapterFixtures`, 16 → 19): фолбэк `cvtsudoers.ws` на штатном sudoers
  Ubuntu — PASS `rules=3;nonstandard=0`; `cvtsudoers.ws` с режимом `0775` и `0757` — `ERROR
  cvtsudoers:untrusted-fallback`; нет ни `cvtsudoers`, ни `cvtsudoers.ws` —
  `ERROR cvtsudoers:execution-failed`. На адаптере до правки первые два
  теста — FAIL, третий фиксирует прежнее поведение. ВМ-прогон нового
  артефакта не выполнялся.
