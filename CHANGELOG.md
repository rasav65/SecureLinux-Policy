...
- Added a negative regression for a version below the minimum.
- Regenerated system release evidence with the version-floor check active.
- Added compatibility evidence generated by the updated gate under
  `jsonschema 4.26.0`.
- Both 4.10.3 and 4.26.0 retain zero runtime/real and emulator/real
  disagreements and validate all five active controls.
- The current roadmap step remains `INDEX_GENERIC_SOURCE_SKELETON_GENERATOR`;
  this follow-up closes 0 FSTEC source rows.
