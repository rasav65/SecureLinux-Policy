...
## Errata draft v0.9.4 — semantic host-state simplification

**Status:** DRAFT v0.9.4-ERRATA  
...
`BUILD_CONTRACT_V0_9_4_ERRATA = ACCEPT` does **not** admit an authoritative builder.
...
2. create/rerun/audit every Phase-A prerequisite not eligible for exact `REUSED`; item 10 is necessarily `RERUN` as `build-environment-v2`;
3. only after the exact 16 current-contract Phase-A items are admitted, implement or re-admit one hash-frozen NON-RELEASE measurement candidate consistent with the v0.9.4 static state;
4. regenerate Phase-B measurement and all four Phase-C dynamic prerequisites under the v0.9.4 identity; item 11 is `build-runtime-v2`;
...
No v0.9.3 contract-bound Phase-B/Phase-C/BUILD-MANIFEST/final-BUILD-INPUTS/§30 VM evidence is current evidence under v0.9.4.
...
- `sha256 = exact SHA-256 of independently accepted v0.9.4 Markdown bytes`.
...
- accepted root Build Contract v0.9.4;
- accepted `build-environment-v2`.
...
Build Contract v0.9.4 retains the staged 16→candidate→4→Phase-D lifecycle: static admission, measurement-candidate implementation, dynamic admission and authoritative admission.
...
Architectural `BUILD_CONTRACT_V0_9_4_ERRATA = ACCEPT` authorizes creation/audit of exactly these 16 static items:
...
The accepted Build Contract v0.9.4 and accepted build-environment contract are exact `both` root inputs and must be pinned by exact SHA in final `BUILD-INPUTS.lock` before authoritative admission.
...
For this v0.9.4 host-state errata specifically:
...
`B0-01` — root Build Contract v0.9.4 id/version/SHA is pinned and consumed as `both`.  
...
## 37. Questions for independent audit of v0.9.4 errata

This is a targeted post-freeze contract audit. The auditor must compare exact v0.9.3 accepted bytes against v0.9.4 and attack the semantic-host-state/runtime-root changes plus unintended downstream inconsistency.

1. Are the accepted v0.9.3 observer R4 semantics retained unchanged except mechanical root/subordinate-version references?
2. Is the exact 15-row v0.9.3 static host-state universe preserved one-to-one as §18.12.2 mapped substates across exactly eight semantic subjects?
3. Can any mandatory mapped or discovered applicable substate be omitted because a syscall/full trace did not show it? Any such path is a blocker.
4. Are discovered-substate descriptor bytes and `substate_id` deterministic/canonical, with ambiguous mapping/unresolved applicability/duplicates fail-closed?
5. Are the eight `SUBSTATE_SET(S)` values pairwise disjoint and does their exact union equal the complete final applicable substate universe?
6. Does every subject require unique-count + sorted-ID exact equality between coverage records and its exact substate set before PASS, so duplicate collapse or partial coverage cannot pass?
7. Does discovery of one new applicable substate invalidate the prior subject aggregate and require full re-closure, even when it maps to an already existing semantic subject?
8. Can different substates of one subject use different mechanisms/dispositions without creating one misleading subject-level disposition?
9. For `VARIED_NONINFLUENTIAL`, is meaningful builder/runtime-observed change proven per exact substate with identical accepted inputs/runtime/toolchain, byte-identical three core outputs and unchanged dependency closure?
10. Can one composite run pair cover multiple substates only when each exact substate has separately bound before/after exposure and meaningful change?
11. Is `ordering_sensitive=true` still limited to `DENIED`/`FIXED`?
12. Are non-syscall exposures including applicable vDSO/auxv/runtime-library/CPU-feature/runtime-seeding state independently covered rather than inferred from syscall absence?
13. Does `build-runtime-v2` require exactly one immutable independently reproducible digest-addressed **builder** runtime root, while keeping the separately admitted observer identity rules intact?
14. Are substate-set/coverage-set digests and null semantics canonically serialized under §26.1 so two conforming implementations given the same admitted bytes agree?
15. Is Phase-C finalization acyclic `11→13→18→19`, with item-19 evidence absent from final `BUILD-INPUTS.lock`, bound to exact item-18 bytes, and new item-19 discoveries forcing re-closure rather than a hidden cycle?
16. Does §34.5 correctly force item-10 RERUN, invalidate v0.9.3 Phase-B/C/generated BUILD-MANIFEST/final BUILD-INPUTS/VM evidence, and leave other Phase-A items only `PENDING_IMPACT_CHECK` rather than silently reused?
17. Apart from explicit host-state/runtime-root/subordinate-version/migration changes and mechanically dependent references/tests/criteria, is unrelated v0.9.3 normative content unchanged?

## 38. Required audit verdict

Return exactly one top-level verdict:

`BUILD_CONTRACT_V0_9_4_ERRATA = ACCEPT`

or

`BUILD_CONTRACT_V0_9_4_ERRATA = REVISE`
...
`ACCEPT` authorizes only the **staged lifecycle in §34** under the v0.9.4 identity:

- §34.5 impact classification of Phase-A prerequisites;
- Phase A current-contract admission, including mandatory RERUN of item 10 as `build-environment-v2`;
- after all 16 are independently admitted, Phase B NON-RELEASE measurement;
- Phase C regeneration/audit of the exact four dynamic prerequisites including `build-runtime-v2`.

`ACCEPT` does **not** itself authorize authoritative-builder status, Step10B under the old v0.9.3 pair model, publication, FSTEC expansion, real dispositions, APPLY or RESTORE.
