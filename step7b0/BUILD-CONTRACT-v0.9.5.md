...
## Errata draft v0.9.5 — BUILD-INPUTS row identity closure

**Status:** DRAFT v0.9.5-ERRATA  
...
**v0.9.5 delta from accepted v0.9.4:** this is a post-freeze errata under §34.5 for one confirmed item-18 defect only. It closes the previously under-specified `BUILD-INPUTS.lock` fields `input_id` and `contract_version`. All v0.9.4 host-state, runtime-root, observer, target, FSTEC-scope, emission/runtime and publication semantics are retained unchanged unless this document states an exact mechanical version/SHA consequence. Because the root Build Contract version/SHA changes, §34.5 invalidation rules apply; no v0.9.4 contract-bound Phase-B/Phase-C evidence is silently relabeled or reused.


---

## 0. Audit disposition and v0.9.4 errata scope

Build Contract v0.9.3 was independently accepted and frozen by exact byte identity. Subsequent Phase-B/Phase-C engineering and four targeted architecture-audit rounds established that the v0.9.3 mandatory `(class_id, channel_kind)` host-state pair model is stronger and more implementation-specific than necessary for the Step 7B.0 security goal, while the security property itself must remain unchanged: **no known or discovered output-affecting non-filesystem host state may remain an implicit unproved build input**.

v0.9.4 is a **post-freeze errata under §34.5**. It retains the accepted v0.9.3 full-syscall observer semantics and changes the host-state proof granularity and mechanically dependent subordinate identities/gates:

1. **Eight mandatory semantic subjects:** the admission-level host-state universe is grouped into exact semantic classes `environment_boundary`, `time`, `randomness_and_runtime_seeding`, `process_and_credentials`, `process_resources`, `namespace_and_hostname`, `cpu_and_scheduler`, `kernel_and_bootstrap`.
2. **Closed substate coverage:** the 15 accepted v0.9.3 static `(class_id, channel_kind)` rows are not discarded. They become the exact initial `MAPPED_STATIC_SUBSTATES` of those eight subjects. Every known mapped and every later discovered applicable substate/exposure requires exact per-substate coverage before its semantic subject can PASS.
3. **No partial-subject PASS:** one varied/fixed/denied component never closes sibling substates. Exact coverage requires a one-to-one machine-checkable equality between the final applicable substate set and coverage-record keys for each subject.
4. **New-substate invalidation:** discovery of a new applicable substate changes the exact subject substate set, immediately invalidates the prior subject PASS, and requires full re-closure. Unresolved applicability is FAIL, not omission.
5. **Heterogeneous mechanisms are explicit:** substates of one semantic subject may use different `DENIED`, `FIXED` or `VARIED_NONINFLUENTIAL` mechanisms. Subject PASS is only the aggregate of PASS for every exact substate.
6. **Ordering safety is preserved:** `ordering_sensitive=true` is a per-substate property and permits only `DENIED` or `FIXED`; `VARIED_NONINFLUENTIAL` remains forbidden.
7. **Non-syscall assurance is preserved:** syscall-trace absence is never closure evidence for vDSO, auxv, runtime-library dispatch, CPU-feature state, runtime seeding or equivalent exposures.
8. **Digest-addressed builder runtime root:** `build-runtime-v2` closes the builder/tool runtime by one immutable independently reproducible digest-addressed runtime root; floating host `/usr/...` runtime inputs are forbidden.
9. **Subordinate identity bump:** Phase-A item 10 becomes `build-environment-v2`; dynamic item 11 becomes `build-runtime-v2`. Their exact bytes/governing invariants change and therefore are not eligible for silent reuse under §34.5.
10. **Accepted v0.9.3 observer errata remains normative:** item-9 R4 positive-admission, `REVIEW_REQUIRED`, full-syscall/full-process-tree, classification-policy and fitness semantics are carried forward unchanged except mechanical root/subordinate-version references.

This errata does **not** change §30.2 VM semantics, the exact 16→candidate→4→Phase-D lifecycle, candidate-source immutability, `MEASUREMENT_BUILDER_PATHS` project-path principle, project-root read-only requirements, external staging, FSTEC scope, emission/runtime separation or publication restrictions.

The v0.9.3 implementation blockers and fail-closed observer requirements remain blockers unless independently closed. v0.9.4 does not waive an earlier requirement merely because the host-state architecture is simplified.
...
Root identity for this draft:

- `build_contract_id = step7b0-build-contract`;
- `build_contract_version = 0.9.5`.

The contract does not embed its own SHA-256.

After independent ACCEPT, the exact accepted Markdown bytes are pinned by the sidecar/admission evidence and then by staged admission evidence and final `BUILD-INPUTS.lock`:
...
`BUILD_CONTRACT_V0_9_5_ERRATA = ACCEPT` does **not** admit an authoritative builder.

It authorizes only the exact staged lifecycle in §34 under the new contract identity:

1. perform explicit §34.5 impact classification for every Phase-A prerequisite;
2. create/rerun/audit every Phase-A prerequisite not eligible for exact `REUSED` after its §34.5 impact classification;
3. only after the exact 16 current-contract Phase-A items are admitted, implement or re-admit one hash-frozen NON-RELEASE measurement candidate consistent with the v0.9.5 static state;
4. regenerate Phase-B measurement and all four Phase-C dynamic prerequisites under the v0.9.5 identity; item 11 is `build-runtime-v2`;
5. only after all 20 prerequisites and all final gates are admitted may a separate explicit authoritative-builder admission be considered.

No v0.9.4 contract-bound Phase-B/Phase-C/BUILD-MANIFEST/final-BUILD-INPUTS/§30 VM evidence is current evidence under v0.9.5.
...
Record identity/version closure is exact for **every** final `BUILD-INPUTS.lock` row:

- `input_id = "build-input-v1:" + role + ":" + canonical_path`, using the exact UTF-8 bytes of the already-validated `role` token and §18.2 `canonical_path`; no case folding, path normalization beyond §18.2, alternate prefix, escaping convention, digest substitution or implementation-generated identifier is permitted;
- `input_id` values are globally unique; because final `canonical_path` values are unique, any duplicate `input_id` is FAIL;
- `contract_version = 0.9.5` for every row;
- in `BUILD-INPUTS.lock`, `contract_version` identifies the exact root Build Contract version governing the row schema/projection semantics. It is **not** a substitute for a referenced subordinate artifact's own internal contract/schema version. Any subordinate version required elsewhere (including BUILD-MANIFEST identity fields) is still read and verified under that artifact's accepted schema;
- `null`, empty, role-specific ad-hoc, filename-derived or implementation-selected `contract_version` values are forbidden.

Thus two conforming implementations given the same final `(role,canonical_path,sha256,consumption_scope)` set must derive byte-identical `input_id` and `contract_version` values before canonical serialization.
...
- `sha256 = exact SHA-256 of independently accepted v0.9.5 Markdown bytes`.
...
### 18.4. `build-environment-v2` is an exact `both` input

Because BUILD-MANIFEST emits `build_environment_contract_id`, `version` and `sha256`, the exact accepted `build-environment-v2` file is mandatory. During measurement-candidate execution its exact id/version/SHA is bound in the measurement evidence envelope; final admission additionally requires:
...
It does **not** claim final `BUILD-INPUTS.lock`, `build-runtime-v2` or `TOOLCHAIN.lock` admission.

**Final authoritative closure** is derived only after dynamic measurement and includes:

- accepted root Build Contract;
- accepted `build-environment-v2`;
- accepted `build-runtime-v2`/runtime lock;
...
1. **final-lock member:** `admission_only=false` and `build_inputs_role` is one exact allowed §18 role. Final `BUILD-INPUTS.lock` must contain exactly one row with the same `canonical_path`, `sha256`, exact `consumption_scope`, and `role == build_inputs_role`; its `input_id` and `contract_version` are then derived exactly by the global §18 rules above;
...
The observer must capture the **full syscall stream** of the candidate process tree. A filtered `%file`, `%process`, named-syscall subset, sampling profiler, or best-effort trace is not admissible closure evidence. The v0.9.3 observer correction, retained unchanged by v0.9.4, changes only evaluation of a previously unknown but structurally trustworthy event; it does not permit filtered capture.
...
- `process_identity_query` — read-only query of process/thread identity; returned host-state values remain independently subject to §18.12;
- `process_resource_query` — read-only query of resource limits/accounting visible to the process; returned host-state values remain independently subject to §18.12.
...
The full syscall observer is necessary but **not sufficient** for host-state closure. §§18.9–18.12 and accepted `build-environment-v2`/`build-runtime-v2` additionally define the mandatory host-state surface, including substates/exposures that may be satisfied without an observable syscall (for example vDSO, auxv, runtime-library bootstrap or direct CPU-feature instructions).

Absence from the syscall trace is never evidence that a mandatory semantic subject or applicable substate does not exist.
...
2. exact accepted `build-environment-v2` path;
...
### 18.10. `build-environment-v2` — exact process environment and write boundary
...
Spike evidence shows an isolated Python baseline is viable, but v0.9.4 does not hard-code guessed runtime file paths from that experiment.
...
### 18.11. `build-runtime-v2` — measured non-project filesystem/tool closure

Non-project runtime dependencies are not guessed from documentation or a hand-written list.

`build-runtime-v2` is a **dynamic prerequisite**. It is created only from a hash-frozen NON-RELEASE measurement candidate after the exact 16 static prerequisites in §34 are independently admitted.
...
4. execute that exact candidate under the accepted full-syscall dependency observer and exact `build-environment-v2` envelope;
...
8. create candidate `build-runtime-v2` and `TOOLCHAIN.lock` identities;
...
10. perturb/remove undeclared external paths and required host-state substates and require either no effect under an admitted disposition or explicit fail-closed behavior;
11. finalize dynamic prerequisites in acyclic order: item 11 `build-runtime-v2` → item 13 `TOOLCHAIN.lock` → item 18 final `BUILD-INPUTS.lock` → item 19 host-state/hermeticity evidence. Item 19 is admission/gate evidence, is not a builder input, and must be absent from final `BUILD-INPUTS.lock`;
12. run final admission again with those dynamic bytes present. Any new external dependency, applicable host-state substate/exposure or changed candidate source SHA resets dynamic admission. If item-19 execution discovers a new applicable substate, invalidate items 11/13/18/19 as necessary and restart dynamic closure from item 11.
...
Host-state closure is defined by an **accepted mandatory semantic surface plus exact closed substate coverage**, not merely by events that happen to appear in a tracer output.

`build-environment-v2` owns:

- the exact eight-value `MANDATORY_SEMANTIC_HOST_STATE_CLASSES` enum;
- the exact `MAPPED_STATIC_SUBSTATES` set derived from the accepted v0.9.3 static row universe;
- the sanitized environment and H1 sandbox boundary.

`build-runtime-v2` owns:

- `DISCOVERED_APPLICABLE_SUBSTATES`;
- the final applicable substate universe;
- the immutable builder runtime-root identity/digest.

Phase-C item 19 owns the exact per-substate coverage records and semantic-subject aggregates bound to that final universe.

#### 18.12.1. Exact mandatory semantic subject universe

`MANDATORY_SEMANTIC_HOST_STATE_CLASSES` is exactly:

1. `environment_boundary`;
2. `time`;
3. `randomness_and_runtime_seeding`;
4. `process_and_credentials`;
5. `process_resources`;
6. `namespace_and_hostname`;
7. `cpu_and_scheduler`;
8. `kernel_and_bootstrap`.

Unknown semantic class is FAIL until this contract is revised and independently admitted.

The mandatory semantic universe applies regardless of whether a syscall is observed. Absence from syscall/full-trace evidence is never a reason to omit a mandatory subject or substate.

#### 18.12.2. Exact 15 mapped static substates

The v0.9.3 accepted static row universe is retained as exact initial substate coverage, not as 15 independent admission subjects.

Each mapped static substate has a globally unique ASCII `substate_id` and exact mapping:

| `substate_id` | legacy `(class_id,channel_kind)` | semantic subject | `ordering_sensitive` |
|---|---|---|---|
| `environment_boundary.caller_environment.environment_bootstrap` | `caller_environment / environment_bootstrap` | `environment_boundary` | `true` |
| `time.time_sources.syscall` | `time_sources / syscall` | `time` | `false` |
| `time.time_sources.vdso` | `time_sources / vdso` | `time` | `false` |
| `time.timezone.environment_bootstrap` | `timezone / environment_bootstrap` | `time` | `false` |
| `randomness_and_runtime_seeding.randomness_entropy.syscall` | `randomness_entropy / syscall` | `randomness_and_runtime_seeding` | `false` |
| `process_and_credentials.credentials.syscall` | `credentials / syscall` | `process_and_credentials` | `false` |
| `process_and_credentials.process_identity.syscall` | `process_identity / syscall` | `process_and_credentials` | `false` |
| `process_resources.resource_limits.syscall` | `resource_limits / syscall` | `process_resources` | `false` |
| `namespace_and_hostname.namespace_cgroup_hostname.procfs_or_sysfs_host_state` | `namespace_cgroup_hostname / procfs_or_sysfs_host_state` | `namespace_and_hostname` | `false` |
| `cpu_and_scheduler.scheduler_cpu_topology.syscall` | `scheduler_cpu_topology / syscall` | `cpu_and_scheduler` | `false` |
| `cpu_and_scheduler.scheduler_cpu_topology.cpu_instruction_or_feature_dispatch` | `scheduler_cpu_topology / cpu_instruction_or_feature_dispatch` | `cpu_and_scheduler` | `false` |
| `cpu_and_scheduler.cpu_feature_dispatch.runtime_library` | `cpu_feature_dispatch / runtime_library` | `cpu_and_scheduler` | `false` |
| `kernel_and_bootstrap.kernel_os_arch.syscall` | `kernel_os_arch / syscall` | `kernel_and_bootstrap` | `true` |
| `kernel_and_bootstrap.kernel_os_arch.auxv` | `kernel_os_arch / auxv` | `kernel_and_bootstrap` | `true` |
| `kernel_and_bootstrap.auxv_bootstrap.auxv` | `auxv_bootstrap / auxv` | `kernel_and_bootstrap` | `false` |

`MAPPED_STATIC_SUBSTATES` is exactly the above 15 `substate_id` values. Missing, duplicate, additional, remapped or differently ordered-sensitive static substate is FAIL unless a later Build Contract version is independently admitted.

#### 18.12.3. Discovered applicable substate descriptor and identity

Measurement may discover additional output-relevant non-filesystem host state.

A discovered applicable substate descriptor is a closed canonical-JSON object with exactly:

- `semantic_class_id`;
- `origin = discovered_applicable`;
- `exposure_authority`;
- `exposure_name`;
- `discovery_evidence_sha256`;
- `ordering_sensitive`.

`semantic_class_id` must be one exact §18.12.1 value.

`ordering_sensitive` is an exact JSON boolean, `true` or `false`; string, integer, null, missing value or implicit default is FAIL. Its value is the authoritative ordering-sensitivity classification for that discovered descriptor. If ordering sensitivity cannot be resolved from the admitted discovery evidence, classification is unresolved and the descriptor is FAIL, not omission.

`exposure_authority` is exactly one of:
...
`exposure_name` is non-empty printable ASCII, contains no control character, `/`, `\`, `..`, leading/trailing whitespace or consecutive whitespace, and is part of the reviewed Phase-C subject bytes.

`discovery_evidence_sha256` is lowercase 64-hex SHA-256 of exact canonical evidence bytes that establish applicability/exposure.

The descriptor is serialized under §26.1 with exactly one final LF.

For discovered substates:

`substate_id = "discovered." + semantic_class_id + "." + SHA256(canonical descriptor bytes)`

The SHA component is lowercase 64-hex. The descriptor does not itself contain `substate_id`, avoiding self-reference.

Two discovered descriptors with the same `substate_id` are duplicates and FAIL. Two different descriptors claimed to represent the same real exposure, ambiguous classification between semantic subjects, unresolved exposure authority/name, or unresolved applicability are FAIL, not omission.

#### 18.12.4. Exact final applicable substate universe

Define:

`FINAL_APPLICABLE_SUBSTATE_UNIVERSE = MAPPED_STATIC_SUBSTATES ∪ DISCOVERED_APPLICABLE_SUBSTATE_IDS`.

The two sets are disjoint; collision is FAIL.

Every final substate belongs to exactly one semantic subject. Define for each subject `S`:

`SUBSTATE_SET(S) = { x ∈ FINAL_APPLICABLE_SUBSTATE_UNIVERSE | semantic_class(x) = S }`.

Machine-checkable union invariants are all mandatory:

- every `substate_id` is globally unique;
- `SUBSTATE_SET(S)` values are pairwise disjoint;
- the union across all exact eight semantic subjects equals `FINAL_APPLICABLE_SUBSTATE_UNIVERSE`;
- the union contains all exact 15 mapped static IDs;
- unresolved applicability is FAIL, not omission;
- unknown/unclassified output-relevant host state is FAIL.

A discovered applicable substate inside an already existing semantic subject still changes that subject's exact `SUBSTATE_SET(S)`.

#### 18.12.5. Canonical substate-set serialization and digest

A substate set is serialized as a JSON array of unique `substate_id` strings sorted by raw UTF-8 byte order, using §26.1 standalone JSON bytes with exactly one final LF.

`substate_set_sha256 = SHA256(those exact bytes)`.

Before digesting:

- input count must equal unique-ID count;
- no duplicate may be collapsed as a set operation;
- sorting occurs only after duplicate rejection.

Two conforming implementations given the same admitted descriptors must therefore produce identical set bytes and digest.

#### 18.12.6. Per-substate coverage record schema

Every final applicable substate has exactly one Phase-C coverage record.

The closed record fields are exactly:
...
- `baseline_run_id`;
- `perturbed_run_id`;
- `baseline_observed_state_sha256`;
- `perturbed_observed_state_sha256`;
- `observed_state_changed`;
- `builder_exposure_evidence_sha256`;
- `baseline_core_output_sha256s`;
- `perturbed_core_output_sha256s`;
- `core_outputs_byte_identical`;
- `baseline_dependency_closure_sha256`;
- `perturbed_dependency_closure_sha256`;
- `dependency_closure_unchanged`;
- `result`.

`source_kind` is exactly `mapped_static` or `discovered_applicable`.

`ordering_sensitive` in a coverage record is an exact JSON boolean and is not an independent authority:

- for `source_kind=mapped_static`, it must equal the authoritative `ordering_sensitive` value of the exact matching §18.12.2 mapped-static row;
- for `source_kind=discovered_applicable`, it must equal the authoritative `ordering_sensitive` value in the exact discovered descriptor referenced by `source_identity_ref`.

Any missing value, non-boolean value or mismatch between the coverage record and its authoritative source is FAIL before disposition-specific evaluation.

`source_identity_ref` is exact:

- for `mapped_static`: the exact literal `mapped_static:` followed by that mapped `substate_id`;
- for `discovered_applicable`: the exact literal `discovered_descriptor_sha256:` followed by SHA-256 of the exact standalone canonical descriptor bytes.

`accepted_input_set_sha256` is SHA-256 of the exact current final `BUILD-INPUTS.lock` standalone bytes. Item 19 is generated only after item 18 and is absent from that lock, so this binding creates no self-cycle.

`runtime_root_sha256` equals the exact `builder_runtime_root_sha256` repeated by `TOOLCHAIN.lock`.

`toolchain_lock_sha256` is SHA-256 of exact current `TOOLCHAIN.lock` standalone bytes.

`disposition` is exactly:

1. `DENIED`;
2. `FIXED`;
3. `VARIED_NONINFLUENTIAL`.

`result` is exactly `PASS` or `FAIL`.

Every SHA field, when non-null, is lowercase 64-hex.

`baseline_core_output_sha256s` and `perturbed_core_output_sha256s`, when non-null, are closed objects with exactly the three keys:

- `securelinux-policy-check.sh`;
- `BUILD-MANIFEST.json`;
- `BUILD-SHA256`.

For `FIXED` and `DENIED`, all variance-only fields are exactly JSON `null`:
...
`mechanism_id`, `mechanism_evidence_sha256`, `builder_exposure_evidence_sha256`, accepted-input/runtime/toolchain bindings and `result` remain mandatory for all dispositions.

For `VARIED_NONINFLUENTIAL` none of the variance fields may be null, and exact conditions in §18.12.8 apply.

Unknown field, missing field or implicit default is FAIL.

#### 18.12.7. Coverage-record set and semantic-subject aggregate

For each semantic subject `S`, let `COVERAGE_RECORDS(S)` be all exact coverage records whose `semantic_class_id == S`.

Before any PASS:

- coverage-record `substate_id` values are unique;
- every record's `semantic_class_id` equals the class of its referenced substate;
- every record's `ordering_sensitive` equals the authoritative value from §18.12.2 or its referenced §18.12.3 descriptor;
- `len(COVERAGE_RECORDS(S)) == len(SUBSTATE_SET(S))`;
- sorted coverage-record IDs equal the exact sorted IDs in `SUBSTATE_SET(S)`;
- no extra/unclassified coverage record exists.

Equivalent shorthand:

`keys(COVERAGE_RECORDS(S)) == SUBSTATE_SET(S)`

but implementations must check uniqueness/count before set comparison; duplicate collapse is forbidden.

Coverage records are serialized as a canonical JSON array sorted by `substate_id` UTF-8 bytes under §26.1 with exactly one final LF.

`coverage_record_set_sha256 = SHA256(those exact bytes)`.

The semantic-subject aggregate is a closed object with exactly:

- `semantic_class_id`;
- `substate_set_sha256`;
- `ordered_substate_ids`;
- `coverage_record_set_sha256`;
- `subject_status`.

`ordered_substate_ids` is the same unique sorted list used to compute `substate_set_sha256`.

`subject_status` is exactly `PASS` or `NOT_ADMITTED`.

`subject_status = PASS` iff:

- exact set/coverage equality above holds;
- every referenced coverage record has `result=PASS`;
- every record satisfies its disposition-specific rules.

There is no single subject-level disposition. Different substates of one subject may use different admitted mechanisms/dispositions.

#### 18.12.8. Disposition semantics

`DENIED` means an accepted sandbox/runtime boundary prevents builder/runtime exposure to that exact substate and the mechanism evidence proves denial from the builder/runtime perspective.

`FIXED` means an accepted sandbox/wrapper/platform/runtime-root mechanism makes the relevant exposed value/identity exact and that value/identity is bound into admitted environment/runtime evidence.

`VARIED_NONINFLUENTIAL` is permitted only for `ordering_sensitive=false` and requires all:

1. exact same accepted byte inputs;
2. exact same runtime-root digest;
3. exact same `TOOLCHAIN.lock` identity;
4. builder/runtime exposure is independently evidenced;
5. baseline and perturbed observed-state canonical evidence bytes differ, therefore their SHA-256 values differ;
6. `observed_state_changed=true`;
7. the three core outputs are byte-identical;
8. `core_outputs_byte_identical=true`;
9. dependency-closure canonical bytes/digests are identical;
10. `dependency_closure_unchanged=true`.

A controlled run pair may support multiple substates only when the evidence contains independently bound before/after exposure for **every** covered `substate_id` and meaningful variation is proven separately for each. One changed component never closes sibling substates.

If `ordering_sensitive=true`, allowed dispositions are only `DENIED` or `FIXED`.

If a variable substate cannot be denied, fixed or (where allowed) meaningfully perturbed, admission is FAIL until a stronger sandbox/runtime design is provided.

#### 18.12.9. New-substate invalidation

Discovery of a new applicable substate after a semantic subject had PASS is not an additive warning.

It:

1. changes `FINAL_APPLICABLE_SUBSTATE_UNIVERSE`;
2. changes exactly one `SUBSTATE_SET(S)`;
3. changes `substate_set_sha256`;
4. invalidates prior `coverage_record_set_sha256`;
5. immediately makes prior `subject_status=PASS` stale/current-status `NOT_ADMITTED`;
6. requires complete coverage for the new exact substate set before PASS can be restored.

A previous subject PASS cannot survive merely because the new substate maps to an existing semantic class.

If new host state cannot be unambiguously represented under the exact descriptor/class rules, Phase C FAILS and a new Build Contract version/SHA is required.

#### 18.12.10. Independent non-syscall assurance

The full syscall observer is necessary but not sufficient.

Syscall trace alone cannot satisfy coverage for:

- vDSO-backed time;
- auxv/bootstrap state;
- runtime-library CPU dispatch;
- direct CPU-feature/dispatch state;
- runtime seeding/entropy not reducible to the observed syscall exposure;
- equivalent kernel/runtime-provided non-syscall state.

Each such applicable substate requires an admitted denial/fix/meaningful-variation mechanism from the actual builder/runtime exposure point.
...
- every mandatory semantic host-state subject from §18.12, not only state observed by a syscall tracer;
- exact coverage for every mapped/discovered applicable substate of each subject;
- representative independent non-syscall exposures including applicable vDSO/auxv/runtime-library/CPU-feature/runtime-seeding paths;
- explicit proof that every `ordering_sensitive=true` substate is `DENIED` or `FIXED`;
- floating external file/version substitution.

Each case must be inaccessible/ignored, fixed/pinned, produce byte-identical output under an allowed per-substate perturbation proof, or explicitly FAIL.

Silent output variation is forbidden. A missing perturbation/control mechanism for any mandatory/applicable substate is FAIL. A successful perturbation of one substate cannot satisfy a different uncovered sibling substate. Discovery of new applicable host state during the gate invalidates the affected semantic-subject aggregate under §18.12.9.
...
- accepted root Build Contract v0.9.5;
- accepted `build-environment-v2`.
...
Non-project interpreter/runtime files are not represented by vague classes such as “system Python”. They are closed by accepted `build-runtime-v2`/`TOOLCHAIN.lock` evidence and exact cryptographic/runtime-root identity according to §18.11.
...
Build-environment identity fields equal the accepted `build-environment-v2` `both` lock row.
...
- build-dependency observer executable SHA-256 and, according to identity mode, exact observer runtime-root digest or `runtime_members_sha256`, all field-wise equal to the admitted Phase-A item-9 identity;
...
- builder interpreter exact executable path **inside the builder runtime root** plus exact interpreter version/SHA-256;
- exact canonical interpreter flags;
- serializer implementation/version if external to builder;
- `build-runtime-v2` identity/digest;
- `builder_runtime_root_sha256`;
- exact executable paths used inside the builder runtime root.

`TOOLCHAIN.lock` must **repeat**, not redefine or re-resolve, the exact Phase-A item-9 observer identity. Equality is checked against the admitted descriptor and fitness evidence. Same tool/version/profile with a different executable SHA, observer runtime identity, parser SHA, profile SHA or classification-policy SHA is FAIL and returns the lifecycle to Phase A before any authoritative admission.

`build-runtime-v2` is derived from measurement, not guessed. For the **builder/tool runtime** it must use exactly one immutable independently reproducible digest-addressed runtime-root identity. The root contains only admitted output-relevant builder/runtime bytes and exact executable paths. Its canonical content identity/digest is bound by `TOOLCHAIN.lock`.

Floating host runtime files, package-name-only identity, mutable host `/usr/...` runtime inputs, “latest”, default config, or unversioned tools are forbidden.

The builder runtime root requirement does not silently absorb non-filesystem host state. CPU/time/auxv/vDSO/credentials/resources/namespaces and equivalent state remain subject to §18.12.

The separately admitted build-dependency observer may retain the exact Phase-A item-9 identity mode already admitted under §18.6.1; v0.9.4 does not silently rewrite that observer identity into the builder runtime root.

Any observed external regular file used by the builder and not inside/bound by the exact runtime-root closure is FAIL.

Changing any pinned runtime/tool/root identity requires rerunning affected build/hermeticity/determinism gates.
...
2. identical accepted `build-environment-v2` bytes;
3. identical accepted `build-runtime-v2`/toolchain identities;
...
- every mandatory/applicable semantic host-state substate requiring variance according to §18.12;
...
Every substate using disposition `VARIED_NONINFLUENTIAL` must show different builder/runtime-observed state across controlled runs while accepted inputs/runtime/tool identities remain exact, dependency closure is unchanged and the three core output files remain byte-identical.
...
Build Contract v0.9.5 retains the staged 16→candidate→4→Phase-D lifecycle: static admission, measurement-candidate implementation, dynamic admission and authoritative admission.
...
Architectural `BUILD_CONTRACT_V0_9_5_ERRATA = ACCEPT` authorizes creation/audit of exactly these 16 static items:
...
10. `build-environment-v2` with sanitized environment, exact interpreter invocation, inherited-FD/network rules, external-staging write boundary, the exact eight semantic host-state subjects and exact 15 `MAPPED_STATIC_SUBSTATES` from §18.12;
...
- candidate runs only under accepted `build-environment-v2` + `build-dependency-observer-v1`;
...
11. measured `build-runtime-v2` with the immutable digest-addressed builder runtime root plus exact discovered-applicable-substate descriptors/final substate universe from §18.12;
...
19. exact per-substate host-state coverage records + semantic-subject aggregates and hermeticity perturbation evidence satisfying §18.12/§18.13 for every mapped/discovered applicable substate.

Dynamic finalization order is exact and acyclic: `11 → 13 → 18 → 19`. Item 19 is admission/gate evidence only and is absent from the final `BUILD-INPUTS.lock` builder input universe. Its `accepted_input_set_sha256` binds the already-finalized item-18 file bytes.
...
The accepted Build Contract v0.9.5 and accepted build-environment contract are exact `both` root inputs and must be pinned by exact SHA in final `BUILD-INPUTS.lock` before authoritative admission.
...
For this v0.9.5 item-18 row-identity errata specifically:

- all 16 Phase-A prerequisite subjects remain `PENDING_IMPACT_CHECK`; each may become `REUSED` only by the existing §34.5 rule after exact subject/member bytes and governing invariants are proven unchanged under v0.9.5;
- Phase-B measurement evidence under v0.9.4 is `INVALIDATED` because the Build Contract SHA in `MEASUREMENT_BUILDER_PATHS` changes;
- the v0.9.4 Phase-B candidate source cannot be silently reused if it embeds the old Build Contract version/SHA; any required mechanical identity update creates a new candidate source SHA and requires a new freeze/measurement lifecycle;
- Phase-C item 11 `build-runtime-v2` and item 13 `TOOLCHAIN.lock` admitted under v0.9.4 are `INVALIDATED` as current evidence and must be regenerated after the new Phase-B measurement; item 18 was not admitted and item 19 was not started;
- previously accepted v0.9.4 evidence remains historical and must not be deleted or relabeled as v0.9.5 evidence.

Historical v0.9.4 host-state errata transition, retained only for provenance:

- Phase-A item 10 is `RERUN`: `build-environment-v1` cannot be reused as current item 10 because the subject identity/governing host-state schema changes to `build-environment-v2`;
- Phase-B measurement evidence is `INVALIDATED` under the changed Build Contract/item-10 byte set and must be regenerated;
- Phase-C items 11/13/18/19 are `INVALIDATED` as current evidence and must be regenerated; item 11 is the new `build-runtime-v2`;
- generated BUILD-MANIFEST, final `BUILD-INPUTS.lock` and §30 VM evidence under v0.9.3 are `INVALIDATED` as current v0.9.4 evidence;
- every other Phase-A item remains `PENDING_IMPACT_CHECK` until individually classified `REUSED` or `RERUN`; this contract does not predeclare them reused;
- the prior measurement-candidate source SHA is not automatically admitted under v0.9.4. Exact unchanged source bytes may be reconsidered only after current Phase-A admission and must undergo new Phase-B measurement; any source-byte change receives a new candidate SHA and the same measurement lifecycle.
...
`B0-01` — root Build Contract v0.9.5 id/version/SHA is pinned and consumed as `both`.  
`B0-02` — accepted build-environment-v2 id/version/SHA is pinned and consumed as `both`.  
...
`B0-37` — BUILD-INPUTS.lock exact required set/role/scope plus §18-derived `input_id`/`contract_version` are reproducible.  
...
`B0-51` — every exact mapped/discovered applicable host-state substate belongs to exactly one mandatory semantic subject and has exactly one accepted per-substate coverage record; absence from syscall trace is not exemption.  
`B0-52` — unknown/unclassified host-state exposure, unresolved applicability, trace/evidence loss, ambiguous semantic mapping or missing substate coverage cannot PASS.  
...
`B0-74` — environment/metadata/path/host-state gates cover the exact mandatory semantic/substate surface and produce admitted denial/fix/byte-identical variance evidence or explicit FAIL.  
...
`B0-87` — mandatory non-syscall substates (including applicable vDSO/auxv/runtime-library/CPU-feature/runtime-seeding exposures) are covered independently of syscall observation.  
...
`B0-89` — every `ordering_sensitive=true` host-state substate is `DENIED` or `FIXED`; `VARIED_NONINFLUENTIAL` is forbidden for such substates.  
...
`B0-92` — the exact 15 mapped static substates plus discovered applicable substates form one globally unique final universe whose pairwise-disjoint eight semantic-subject sets have exact union equality and exact coverage-record equality; duplicate collapse or unresolved applicability cannot PASS.  
...
`B0-97` — every non-admission-only Phase-A member projects to exactly one final `BUILD-INPUTS.lock` row with exact path/hash/scope/role and exact §18-derived `input_id`/`contract_version`, while every admission-only member path is absent from final `BUILD-INPUTS.lock`.  
...
`B0-113` — `MANDATORY_SEMANTIC_HOST_STATE_CLASSES` is exactly the eight-value §18.12.1 enum and `MAPPED_STATIC_SUBSTATES` is exactly the 15-row §18.12.2 set; every static substate mapping/ordering flag is exact.  
`B0-114` — every discovered applicable substate has an exact closed canonical descriptor and deterministic `substate_id`; ambiguous mapping, unresolved applicability, duplicate/colliding descriptor or unknown semantic class cannot PASS.  
`B0-115` — per-subject substate IDs are unique, subject sets are pairwise disjoint, and their exact union equals the complete final applicable substate universe including all 15 mapped static IDs.  
`B0-116` — for every semantic subject, coverage-record IDs are unique and count/sorted-ID equality with the exact `SUBSTATE_SET(S)` holds before `subject_status=PASS`; missing/extra/duplicate/ambiguous records cannot PASS.  
`B0-117` — a newly discovered applicable substate changes the bound substate-set digest, invalidates the prior subject aggregate/coverage-set digest and requires full re-closure before PASS can be restored.  
`B0-118` — `VARIED_NONINFLUENTIAL` evidence is bound per exact substate, proves different builder/runtime-observed state with identical accepted inputs/runtime/tool identities, byte-identical three core outputs and unchanged dependency closure; a composite run covers only substates individually proven changed.  
`B0-119` — substate-set, coverage-record-set and discovered-descriptor canonical JSON serialization/digests are exact under §26.1; duplicates are rejected before sorting/digesting and canonical JSON `null` is mandatory for inapplicable variance fields.  
`B0-120` — builder/tool runtime closure uses one immutable independently reproducible digest-addressed runtime root bound by `build-runtime-v2`/`TOOLCHAIN.lock`; floating host runtime files or mutable `/usr/...` identities cannot be admitted as builder runtime inputs.  
`B0-121` — Phase-C dynamic finalization is acyclic `11→13→18→19`; item-19 evidence is absent from final `BUILD-INPUTS.lock` and binds exact item-18 bytes, while any new substate discovered during item-19 execution invalidates affected dynamic closure and restarts from item 11.  
`B0-122` — `ordering_sensitive` has exactly one authoritative source per substate: the exact §18.12.2 mapped-static row or the exact referenced §18.12.3 discovered descriptor; every coverage record must repeat that exact JSON boolean, unresolved discovered ordering classification or any source↔record mismatch is FAIL before disposition evaluation.  
...
- require `build-runtime-v2` before Phase-B candidate exists and verify the cycle is rejected;
...
Mechanically verify the exact §18.12.2 15-row legacy mapping to the eight semantic subjects. Remove, duplicate, remap or change ordering sensitivity of any mapped static substate; each must FAIL.

For every mandatory semantic subject attempt at least one representative output-relevant exposure, including:

- caller environment/bootstrap;
- syscall and vDSO time plus timezone;
- randomness/runtime seeding;
- uid/gid and pid/ppid identity;
- resource limits;
- namespace/cgroup/hostname identity;
- affinity/CPU topology, direct CPU-feature dispatch and runtime-library dispatch;
- uname/kernel/architecture and auxv/bootstrap state.

Try omitting a mandatory mapped substate solely because syscall trace did not show it; this must FAIL.

Create a discovered applicable substate inside an already existing semantic subject after that subject previously PASSed. Require:

- changed `SUBSTATE_SET(S)`;
- changed set digest;
- prior aggregate becomes stale/`NOT_ADMITTED`;
- new complete per-substate coverage before PASS can return.

For a multi-substate subject such as `cpu_and_scheduler`, vary only CPU affinity/count while leaving direct CPU-feature and runtime-library dispatch substates uncovered. Subject PASS must FAIL.

Try using one composite run pair to claim coverage of multiple substates when machine-checkable before/after exposure exists for only one. Unproved sibling coverage must FAIL.

Try duplicate coverage-record IDs that would collapse under naive set conversion. Require FAIL before sorting/digesting.

Try unresolved applicability or ambiguous mapping of a discovered exposure. Require FAIL, not omission.

Try `VARIED_NONINFLUENTIAL` on an `ordering_sensitive=true` substate; require FAIL.

For a mapped-static substate whose §18.12.2 authoritative value is `ordering_sensitive=true`, put `ordering_sensitive=false` in its coverage record and attempt `VARIED_NONINFLUENTIAL`; require FAIL on source↔coverage mismatch before disposition evaluation.

For a discovered descriptor, use missing/null/string/integer `ordering_sensitive`, then use a valid JSON boolean in the descriptor but the opposite boolean in its coverage record; every case must FAIL. Unresolved discovered ordering classification must FAIL, not omission.

For `VARIED_NONINFLUENTIAL`, keep observed relevant state equal while outputs are byte-identical; require FAIL. Change state only outside the builder/runtime exposure point without bound exposure evidence; require FAIL.

Hide a dependency in vDSO/auxv/runtime-library/CPU-feature/runtime-seeding state not visible to syscall filtering; syscall-trace absence alone must never close it.

Substitute a floating host runtime file outside the digest-addressed builder runtime root; require FAIL.
...
## 37. Questions for independent audit of v0.9.5 errata

This is a targeted post-freeze audit of one defect only. Do not re-audit accepted v0.9.4 host-state/runtime-root architecture unless the new bytes directly invalidate it.

Decision point:

1. Does §18 now derive exactly one `input_id` for every final row from exact `role` + `canonical_path`?
2. Is `contract_version` exactly `0.9.5` for every final row, with no alternate per-role interpretation?
3. Does §18.5.1 projection now close all six required row fields?
4. Are B0-37/B0-97 consistent with those rules?
5. Does §34.5 correctly invalidate v0.9.4 Phase-B/Phase-C evidence after the Build Contract SHA/version change without claiming unrelated Phase-A reruns before impact checks?

`ACCEPT` authorizes only the staged lifecycle in §34 under the v0.9.5 identity. It does not admit item 18, an authoritative builder, publication, FSTEC expansion, APPLY or RESTORE.
