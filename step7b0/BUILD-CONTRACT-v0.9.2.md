...
## Frozen working contract v0.9.2 — implementation entry
...
## 0. Audit disposition and v0.9.2 corrections

Build Contract v0.9.1 received mixed independent verdicts. Its conservative status is **REVISE** because one concrete ambiguity remained around observer fitness-launch identity binding. The previously closed staged lifecycle, runtime/order/hermeticity architecture, item-20 evidence, independent Phase-A admission, VM outcome semantics, byte-drift invalidation and errata rules remain unchanged.

v0.9.2 is the final narrow pre-freeze patch and changes only three targeted areas:

1. **Observer fitness launch identity:** the actual executable/runtime/parser/profile byte equalities are recomputed immediately before **every observer launch**, explicitly including every §18.6.1a fitness-suite launch as well as Phase-B and Phase-D launches. Fitness evidence records an exact pre-launch identity verification PASS derived from the actual launched object/runtime, not from descriptor fields alone.
2. **`runtime_members_sha256` serialization:** the digest uses the exact §18.5.3 standalone canonical JSON-value rule, including exactly one final LF, eliminating the remaining one-byte serialization ambiguity.
3. **VM installation-variant evidence boundary:** `installation_variant` is a compatibility-test environment label, not installer provenance and not a production security property. Each required VM run binds an exact digest of its actually installed package set; the required `server` and `server-minimized` runs must have different package-set digests. The Build Contract does not attest which ISO, installer path or image class produced those package sets; correct laboratory preparation belongs to the VM test procedure/workpack.

The v0.5–v0.9 foundations remain normative: external staging only, exact `both` root/environment identities, no floating external values, emission/runtime separation, staged measured-runtime closure, field-wise runtime-row↔block equality, full builder syscall capture plus non-syscall surface, supported-target exact `/proc/sys` attribution, `HOST_STATE_ROW_KEY=(class_id,channel_kind)`, closed Phase-A member roles, and `DENIED|FIXED` only for ordering-sensitive host state.
...
`BUILD_CONTRACT_V0_9_2 = ACCEPT` does **not** admit an authoritative builder.
...
- `sha256 = exact SHA-256 of independently accepted v0.9.2 Markdown bytes`.
...
#### 18.6.1. Full-syscall requirement and cryptographic execution identity

The observer must capture the **full syscall stream** of the candidate process tree. A filtered `%file`, `%process`, named-syscall subset, sampling profiler, or best-effort trace is not admissible closure evidence.
...
Every captured syscall/event must classify exactly once as admitted read-only/bootstrap behavior, admitted write to an explicit staging/stdout/stderr sink, or forbidden behavior. Unknown syscall, undecodable arguments, uncertain FD/path/process attribution, parser uncertainty, dropped/lost/truncated trace, or observer startup failure => FAIL.

#### 18.6.1a. Observer fitness is cryptographically bound admission evidence

Normative fail-closed wording is insufficient by itself. Phase-A item 9 is not admitted until the exact cryptographic observer identity from §18.6.1 passes a machine-executed fitness suite. The suite includes a positive synthetic process/trace with a known exact event inventory and negative fixtures in which each of the following must produce FAIL:

- observer startup failure;
- injected trace truncation or explicit event loss;
- unknown syscall/event record;
- undecodable syscall arguments;
- uncertain FD→path or process attribution;
- parser uncertainty/ambiguous record.
...
The exact item-9 `build_dependency_observer_fitness_evidence` member uses closed `build-dependency-observer-fitness-evidence-v1` fields:

- `fitness_evidence_contract_version = build-dependency-observer-fitness-evidence-v1`;
...
No unknown field is permitted. `observer_identity_descriptor_sha256` is SHA-256 of the exact admitted identity-descriptor member bytes. `observer_executable_sha256`, `runtime_members_sha256`, contract/parser/profile hashes, and `runtime_root_sha256` when applicable equal the descriptor field-wise; `runtime_root_sha256=null` in `binary_closure` mode. `fitness_fixture_set_sha256` is the §18.5.3 `fixture_set_sha256` computed over exactly every admitted item-9 `build_dependency_observer_fitness_fixture` member. The exact `member_role` is therefore part of the digested canonical representation.

`prelaunch_identity_verification` must equal exact token `PASS` and is valid only if the §18.6.1 actual-byte identity verification succeeded immediately before every observer launch used by the fitness suite. It is derived from the actual launch target/runtime and cannot be populated from descriptor equality alone. `positive_inventory_result` must equal exact token `PASS`. `negative_case_results` is a closed object with exactly these keys, each equal exact token `EXPECTED_FAIL_OBSERVED_FAIL`: `startup_failure`, `trace_loss_or_truncation`, `unknown_event`, `undecodable_args`, `uncertain_attribution`, `parser_uncertainty`.

Fitness evidence is reusable only for that exact cryptographic identity. Any observer executable/runtime/config/parser/profile byte change invalidates item 9 admission and requires rerunning the fitness suite before Phase B.
...
Spike evidence shows an isolated Python baseline is viable, but v0.9.2 does not hard-code guessed runtime file paths from that experiment.
...
9. rerun the **same candidate source SHA** under the candidate closure and require observed closure ⊆ admitted closure with no unknown event/channel;
...
- accepted root Build Contract v0.9.2;
- accepted `build-environment-v1`.
...
`TOOLCHAIN.lock` must **repeat**, not redefine or re-resolve, the exact Phase-A item-9 observer identity. Equality is checked against the admitted descriptor and fitness evidence. Same tool/version/profile with a different executable SHA, runtime-root digest, `runtime_members_sha256`, parser SHA or profile SHA is FAIL and returns the lifecycle to Phase A before any authoritative admission.
...
Build Contract v0.9.2 retains the v0.7/v0.8/v0.9 staged lifecycle unchanged: static admission, measurement-candidate implementation, dynamic admission and authoritative admission.
...
Architectural `BUILD_CONTRACT_V0_9_2 = ACCEPT` authorizes creation/audit of exactly these 16 static items:
...
9. `build-dependency-observer-v1` **closed cryptographic bundle/admission unit**, including exact observer contract, `build-dependency-observer-execution-identity-v1` descriptor, exact parser, exact profile/selector bytes, exact observer executable/runtime/config cryptographic identity, §18.6.1a fitness fixtures and fitness evidence bound to that same identity;
...
- the Phase-B measurement envelope binds exact `MEASUREMENT_BUILDER_PATHS` from §18.8, including all builder/both Phase-A bundle members and the earlier-admitted control manifest + five canonical control YAMLs;
...
After these four are drafted, rerun the same candidate source SHA under the proposed final dynamic closure. Any new dependency/channel, changed source SHA, trace loss, unknown event or failed perturbation invalidates Phase C.

Additionally, the accepted Phase-B measurement envelope records the exact SHA-256 of every `MEASUREMENT_BUILDER_PATHS` member. If the SHA-256 of **any** such member changes, prior Phase-B measurement evidence and all Phase-C dynamic prerequisite admissions derived from it are invalidated even when the canonical path set is identical and the candidate source SHA itself is unchanged. Phase B measurement and Phase C derivation/rerun must then be repeated against the new exact byte set before Phase D eligibility can be considered.
...
The accepted Build Contract v0.9.2 and accepted build-environment contract are exact `both` root inputs and must be pinned by exact SHA in final `BUILD-INPUTS.lock` before authoritative admission.
...
An accepted Build Contract is immutable by byte identity. A real defect found after freeze is corrected only by a new version/SHA (for example a later `0.9.2-errata`); accepted contract bytes are never edited in place under the old identity.
...
`B0-01` — root Build Contract v0.9.2 id/version/SHA is pinned and consumed as `both`.  
...
`B0-86` — builder observer profile is full-syscall, full-process-tree, exact-pinned, and loss/unknown/undecodable events fail closed.  
...
`B0-93` — Phase-A item 9 observer fitness suite proves exact positive event inventory and fail-closed startup/loss/unknown/undecodable/uncertain-attribution/parser-uncertainty behavior before Phase B.  
`B0-94` — Phase-A item 9 binds the exact observer executable/runtime/config cryptographic identity used by fitness and Phase B; same version with changed executable/runtime/config/parser/profile identity cannot PASS.  
`B0-95` — Phase-C `TOOLCHAIN.lock` repeats the exact admitted Phase-A item-9 observer identity field-wise/digest-wise; it cannot re-resolve identity from version/profile labels.  
...
`B0-98` — immediately before every observer launch, explicitly including every §18.6.1a fitness-suite launch and every Phase-B/Phase-D launch, SHA-256 of the actual resolved executable bytes equals admitted `executable_sha256`; runtime-root/runtime-member/parser/profile actual bytes are reverified under §18.6.1, and fitness evidence records `prelaunch_identity_verification=PASS` only from those actual pre-launch checks.  
...
- feed an unknown syscall/event record and require FAIL;
...
## 37. Questions for independent audit of v0.9.2

This is a targeted re-audit. The auditor must focus on the three v0.9.2 changes and bytewise regression of unaffected text.

1. Does §18.6.1 require actual executable/runtime/parser/profile identity verification immediately before **every observer launch**, explicitly including every fitness-suite launch, and can fitness evidence PASS only with `prelaunch_identity_verification=PASS` derived from the actual launch object/runtime?
2. Does `runtime_members_sha256` have exactly one canonical byte representation, including exactly one final LF, with no conforming no-LF/CRLF/pretty alternative?
3. Is `installation_variant` now only compatibility-test environment identity, with each required VM evidence package binding an exact installed-package-set digest and the two required variant digests required to differ?
4. Is installer/image provenance explicitly outside Build Contract security semantics and left to the laboratory VM test procedure/workpack, without weakening the requirement for two distinct test environments?
5. Are all unaffected v0.9.1 invariants bytewise unchanged apart from version/cross-reference text mechanically required by v0.9.2?
6. Does any remaining interpretation permit a wrong fitness executable, ambiguous runtime-members serialization, or one identical package-set environment relabeled as both required variants to PASS?

## 38. Required audit verdict

Return exactly one top-level verdict:

`BUILD_CONTRACT_V0_9_2 = ACCEPT`

or

`BUILD_CONTRACT_V0_9_2 = REVISE`
...
- Phase A: creation/audit of the exact 16 static prerequisites under the closed v0.9.2 member schema;
