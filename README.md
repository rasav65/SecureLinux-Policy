...
> Инструмент проверки и безопасного применения настроек Debian и Ubuntu по требованиям ФСТЭК, представленным в политике проекта. CHECK работает без изменения системы; автоматический APPLY реализован для параметров `sysctl`, режимов доступа файлов `/etc/passwd`, `/etc/group`, `/etc/shadow`, системных файлов заданий cron, SUID/SGID-приложений, стандартных системных путей, файлов запуска, части параметров ядра в командной строке загрузки (через GRUB, вступают в силу после перезагрузки), доступа к `su` через группу `wheel` и директив входа по SSH (`PermitEmptyPasswords`, `PermitRootLogin`, `PasswordAuthentication`).

![Статус](https://img.shields.io/badge/status-product%20candidate-orange) ![ОС](https://img.shields.io/badge/ОС-Debian%2012%2F13%20%7C%20Ubuntu%2022.04%2F24.04%2F26.04-informational) ![CHECK](https://img.shields.io/badge/CHECK-read--only-blue) ![APPLY](https://img.shields.io/badge/APPLY-sysctl%20%7C%20file%20modes%20%7C%20GRUB%20%7C%20su%20%7C%20SSH-green) ![Лицензия](https://img.shields.io/badge/license-MIT-lightgrey)
...
| Типовые ошибки конфигурации (`fstec-configuration-2026`) | 49 | 1 | 43 | 5 |
| Регистрация событий безопасности (`fstec-logging-2025`) | 14 | 0 | 5 | 9 |
...
Закрыто 335 из 349 строк индекса (95%). Контролей 52, из них с автоматическим исправлением (APPLY) — 43 (82%).
...
[Прогресс](#прогресс) · [Скачать](#скачать) · [Быстрый старт](#быстрый-старт) · [Возможности](#возможности) · [Применение изменений](#применение-изменений) · [Поддерживаемые системы](#поддерживаемые-системы) · [Документация](#документация)
...
- автоматический APPLY параметров `sysctl`, режимов доступа файлов учётных записей, системных файлов заданий cron, SUID/SGID-приложений, стандартных системных путей и файлов запуска, параметров ядра в командной строке загрузки, доступа к `su` и директив входа по SSH;
...
Автоматический APPLY выполняется механизмами `config-line-with-runtime-v1` (sysctl), `file-mode-owner-v1` (режим файлов), `optional-file-root-files-mode-v1` (режимы системных файлов cron), `suid-sgid-applications-mode-v1` (режимы SUID/SGID-приложений), `standard-system-paths-mode-v1` (режимы стандартных системных путей), `startup-files-write-protection-v1` (режимы файлов запуска), `kernel-cmdline-grub-v1` (параметры ядра в командной строке загрузки: файл `/etc/default/grub.d/zz-securelinux-policy.cfg` и `update-grub`, вступают в силу после перезагрузки; три `iommu` 2.4.5 пишутся (решение 26.09.2026); `mitigations`, `tsx` и `debugfs` не пишутся и выводятся блоком «требуется решение администратора»), `pam-wheel-su-v1` (доступ к `su` только для группы `wheel` с `root`: `/etc/pam.d/su` меняется, только если совпадает с файлом пакета, и только при наличии пользователей в группе `sudo` или `admin`; иначе — блок «требуется решение администратора») и `sshd-config-option-v1` (директивы `PermitEmptyPasswords`, `PermitRootLogin`, `PasswordAuthentication` со значением `no` правкой на месте в `/etc/ssh/sshd_config` и `sshd_config.d`, проверка `sshd -t` и перезагрузка `ssh.service`; при `Match` с другим значением, без пользователя в `sudo`/`admin` или без его ключа SSH — блок «требуется решение администратора»); состав берётся из APPLY registries, количества — из машинного статуса ниже. Остальные controls могут участвовать в CHECK, но не изменяются автоматически без явно поддерживаемой APPLY-семантики.
...
OPEN_INDEX_ROWS=14
CLOSURE_RATIO=335/349
CANONICAL_CONTROLS=52
CLOSURE_CONTRACT_ROWS=41
ADAPTER_KINDS=18
