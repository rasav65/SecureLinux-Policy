...
- автоматический APPLY параметров `sysctl`, режимов доступа файлов учётных записей, системных файлов заданий cron, SUID/SGID-приложений и стандартных системных путей;
...
Автоматический APPLY выполняется механизмами `config-line-with-runtime-v1` (sysctl), `file-mode-owner-v1` (режим файлов), `optional-file-root-files-mode-v1` (режимы системных файлов cron), `suid-sgid-applications-mode-v1` (режимы SUID/SGID-приложений) и `standard-system-paths-mode-v1` (режимы стандартных системных путей); состав берётся из APPLY registries, количества — из машинного статуса ниже. Остальные controls могут участвовать в CHECK, но не изменяются автоматически без явно поддерживаемой APPLY-семантики.
