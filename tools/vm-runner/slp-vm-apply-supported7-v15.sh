...
EXPECTED_CHECK_SHA256=${EXPECTED_CHECK_SHA256:-d840ede3b3887ef498c4baf30cd5daf1052e186342ac6bfcc287b87ce42bc8aa}
EXPECTED_STATES=7
...
NAME=slp-vm-apply-supported7-v15
...
sha256sum -c --quiet securelinux-policy.sh.sha256 >/dev/null 2>&1 || { echo "CHECK_SCRIPT_SHA=MISMATCH" >env1.txt; exit 5; }
{
    ...
    sudo -n ls -la /etc/default/grub.d
} >env1.txt 2>&1
sudo -n /bin/bash -p ./securelinux-policy.sh --check --format raw >check-before.txt 2>>err1.txt
echo "CHECK_RC=$?" >>check-before.txt
sudo -n /bin/bash -p ./securelinux-policy.sh --apply --dry-run >dryrun.txt 2>>err1.txt
echo "APPLY_RC=$?" >>dryrun.txt
sudo -n /bin/bash -p ./securelinux-policy.sh --apply >apply.txt 2>>err1.txt
echo "APPLY_RC=$?" >>apply.txt
sudo -n cat /var/log/securelinux-policy/report.json >apply-report.json 2>>err1.txt
{
    echo '=== DROPIN'
    sudo -n cat /etc/default/grub.d/zz-securelinux-policy.cfg
    echo '=== GRUB_CFG_LINUX'
    sudo -n grep -E '^[[:space:]]*linux[[:space:]]' /boot/grub/grub.cfg | cut -c1-300
} >>env1.txt 2>&1
sudo -n true >/dev/null 2>&1; echo "SUDO_N_AFTER_RC=$?" >>env1.txt
# Кэш sudo привязан к терминалу этой сессии: перезагрузка планируется здесь, через 20 с,
# чтобы ПК успел забрать файлы.
sudo -n systemd-run --quiet --on-active=20 /bin/systemctl reboot >>err1.txt 2>&1
...
sudo -n /bin/bash -p ./securelinux-policy.sh --check --format raw >check-after.txt 2>>err2.txt
echo "CHECK_RC=$?" >>check-after.txt
sudo -n /bin/bash -p ./securelinux-policy.sh --apply >apply2.txt 2>>err2.txt
echo "APPLY_RC=$?" >>apply2.txt
sudo -n cat /var/log/securelinux-policy/report.json >apply2-report.json 2>>err2.txt
sudo -n true >/dev/null 2>&1; echo "SUDO_N_AFTER_RC=$?" >>env2.txt
...
for c in VBoxManage python3 ssh scp timeout tar sha256sum awk stat wc cp cmp; do
...
while IFS=$'\t' read -r -u 3 ENV VM VUUID VMAC SNAP SUUID SSH_TARGET SSH_HOST; do
    ...
    open_master() {
        ...
        chmod 700 "$CTL_DIR"
        CTL="$CTL_DIR/master"
        ssh -M -S "$CTL" -o ControlPersist=900 -o ConnectTimeout=8 -fnNT "$SSH_TARGET" </dev/tty || return 1
    ...
    if [ "$state_rc" -eq 0 ]; then
        if open_master 1; then
            ...
            scp -q -o ControlPath="$CTL" "$PHASE1" "$PHASE2" \
            ...
            if [ "$state_rc" -eq 0 ]; then
                ...
                ssh -tt -S "$CTL" "$SSH_TARGET" "/bin/bash '$REMOTE_DIR/phase1.sh' '$REMOTE_DIR'" </dev/tty || state_rc=1
            fi
    ...
    if [ "$state_rc" -eq 0 ]; then
        if open_master 2; then
            ...
            ssh -tt -S "$CTL" "$SSH_TARGET" "/bin/bash '$REMOTE_DIR/phase2.sh' '$REMOTE_DIR'" </dev/tty || state_rc=1
...
printf 'CANDIDATE_SHA256=%s\nRUNNER=%s\nSTART_FROM=%s\nSTOP_AT=%s\nONLY=%s\n' "$EXPECTED_CHECK_SHA256" "$NAME" "$START_FROM" "$STOP_AT" "$ONLY" >"$RESULTS/00-identity.txt"
