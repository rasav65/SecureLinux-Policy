...
NAME=slp-vm-apply-supported7-v16
...
while IFS=$'\t' read -r -u 3 ENV VM VUUID VMAC SNAP SUUID SSH_TARGET SSH_HOST; do
    ...
    open_master() {
        ...
        until SLP_VM_PW=$SLP_VM_PW SSH_ASKPASS=$ASKPASS SSH_ASKPASS_REQUIRE=force \
            ssh -M -S "$CTL" -o ControlPersist=900 -o ConnectTimeout=8 -o NumberOfPasswordPrompts=1 -fnNT "$SSH_TARGET" </dev/null; do
            [ "$_try" -ge 5 ] && return 1
            _try=$((_try + 1))
            sleep 5
        done
    ...
    if [ "$state_rc" -eq 0 ]; then
        if open_master 1; then
            ...
            if [ "$state_rc" -eq 0 ]; then
                ...
                printf '%s\n' "$SLP_VM_PW" | ssh -S "$CTL" "$SSH_TARGET" "/bin/bash '$REMOTE_DIR/phase1.sh' '$REMOTE_DIR'" || state_rc=1
            fi
    ...
    if [ "$state_rc" -eq 0 ]; then
        if open_master 2; then
            ...
            printf '%s\n' "$SLP_VM_PW" | ssh -S "$CTL" "$SSH_TARGET" "/bin/bash '$REMOTE_DIR/phase2.sh' '$REMOTE_DIR'" || state_rc=1
