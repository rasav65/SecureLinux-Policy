# SecureLinux-Policy — политика переноса инженерного донора
...
и статус каждой строки берутся только из `ROADMAP.tsv`; человекочитаемый current
checkpoint показывает `PROJECT-MAP.md`. Изменение roadmap должно сначала менять
