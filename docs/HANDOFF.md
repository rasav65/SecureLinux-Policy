...
1. 2.3.2: ВМ-прогон 7 сред на кандидате с повтором наблюдения (правило ниже, в
   «Закрыто»); ожидание — CHECK без `ERROR` на всех средах.
...
- 2.3.2: наблюдение, прерванное сменой популяции процессов (`RETRY_REASONS` адаптера),
  ...
  правило вердикта для отдельного наблюдения не меняется.
