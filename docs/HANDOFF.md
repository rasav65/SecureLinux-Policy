...
1. Step 7B `FSTEC_AND_CORPORATE_INDEX_EXPANSION_DISPOSITIONS`: 14 OPEN-строк
   ...
   (с 26.09.2026 — автоматический APPLY, принят ВМ-прогоном 26.09.2026),
...
0. Выполнено: 2.4.5 — `iommu=force`, `iommu.strict=1`, `iommu.passthrough=0` пишутся
   автоматически (`kernel-cmdline-grub-v1`, AUTO). ВМ-прогон 26.09.2026 runner v18 на 7 средах
   принят (кандидат `2b0a68eb…28b5`, архив `slp-vm-apply-supported7-v18-states1-7-20260926-230325.tar.gz` SHA `be4bff57…d1fc`): все среды
   загрузились после APPLY, в `/proc/cmdline` три токена, CHECK 2.4.5 — PASS на 7 средах, FAIL
   после APPLY: Ubuntu — 6 (2.3.10, 2.3.11, 2.4.7, 2.5.3, 2.5.9, 2.6.6), Debian — 4 (2.3.10,
   2.4.7, 2.5.3, 2.5.9). В том же прогоне CHECK до APPLY на Ubuntu 26.04 mini: 2.3.2 `ERROR
   proc-status:no-exe-unclassified` — процесс без `exe`, не поток ядра и не зомби (вероятно,
   завершающийся во время обхода; причина не подтверждена); причина не входит в
   `RETRY_REASONS` адаптера 2.3.2 — открытый вопрос, после APPLY `ERROR` нет. Проверка
   пользователем на Ubuntu 24.04 full
