...
Обязательный release gate с реальным `jsonschema` уже закрыт. Этот observation-контракт сохраняется как действующий инженерный контракт; текущий этап проекта определяется только `ROADMAP.tsv`.
