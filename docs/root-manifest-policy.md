...
При отказе `--write` возвращает все файлы, видимые Git, к байтам и режимам до запуска и удаляет
созданные им файлы.
`truth_sha256` считается по файлам `TRUTH_INPUTS` без колонок `sha256` и `*_sha256`: смена
одних пинов байтов (правка адаптера, контракта, control-yaml) его не меняет, смена состава
(строки, идентификаторы, пути, статусы, ожидаемые значения) — меняет.
