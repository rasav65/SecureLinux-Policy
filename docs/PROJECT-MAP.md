...
`строки source=349 · controlled CLOSED=41 · OPEN=14 · canonical controls=52 · adapters=18 · target-family=linux-x86_64-supported-v1`
...
Тем же исходом `kernel-cmdline-grub-v1` возвращает три параметра загрузки, которые по решению человека 24.09.2026 не пишутся автоматически: `mitigations=auto,nosmt`, `tsx=off`, `debugfs=off`; `iommu=force`, `iommu.strict=1`, `iommu.passthrough=0` пишутся автоматически по решению пользователя 26.09.2026 (ВМ-прогон на семи средах — после этого изменения). `pam-wheel-su-v1` возвращает решение администратору, если `/etc/pam.d/su` отличается от файла пакета или в группах `sudo` и `admin` нет пользователей (решение человека 25.09.2026). `sshd-config-option-v1` возвращает решение администратору, если в области `Match` директива задана не `no`, для `PermitRootLogin` — нет пользователя в `sudo`/`admin`, кроме root, для `PasswordAuthentication` — ни у одного такого пользователя нет ключа SSH (решение 25.09.2026).
...
шаблон механизма отработан, APPLY вне горизонта 1 вместе с классом G3. APPLY для `SRC-0001` выведен
