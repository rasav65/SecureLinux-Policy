...
(значение действующей строки, иначе шаблон `#<Key>`, иначе новая строка перед `Match`);
подготовленные файлы проверяются `sshd -t` до замены, затем `sshd -t` дерева, перезагрузка
`ssh.service` и итоговая проверка `sshd -T`; при ошибке прежние байты заменённых файлов
возвращаются и сверяются, sshd перезагружается повторно. Тест покрывает и отказы:
владелец не root, FIFO, правила `Include` CHECK, глубину `Include`, короткую запись,
ошибку и несовпадение при восстановлении, ошибки чтения при итоговой проверке и сверке,
ошибку удаления временного файла и восстановительной замены, ошибку подготовки временного
файла вместе с ошибкой его удаления, ошибки `close`, FIFO вместо `/etc/group`/`/etc/passwd`,
разделение строк только по LF (как у CHECK), сохранение SUID/SGID, `Match User` администратора.
...
Повтор наблюдения 2.3.2 проверен прогоном 25.09.2026 runner v17 на кандидате
`dcf6ceaf…ef28` (архив `slp-vm-apply-supported7-v17-states1-7-20260925-175302.tar.gz`
SHA `fee52469…2276`): CHECK до и после APPLY на 7 средах — `CHECK_RC=0`, без `ERROR`,
2.3.2 — PASS во всех 14 проверках; счётчики CHECK, APPLY и режимы фикстур совпадают
с прогоном `b82a9d8c…3ead`; stderr пуст.
Разведка для Step 7B — read-only прогон 25.09.2026 сценарием `slp-vm-probe-supported7-v1.sh`
(вне репозитория; архив `slp-vm-probe-supported7-v1-states1-7-20260925-211959.tar.gz` SHA `28c7202a…3a98`, 17 файлов, в evidence): на 7 средах
собраны пакеты, `login.defs`, PAM, `sshd -T`, auditd, journald и слушающие порты; сбор
выполняется на закреплённом снимке каждой среды. Выводы — в очереди `docs/HANDOFF.md`.
...
тест кладёт объект в каталог с правами `000` и запускает CHECK непривилегированным
пользователем; до проверки зондом доказывается, что объект действительно недоступен. Ожидание —
ERROR с причиной, уже заданной в коде адаптера; парный тест с отсутствующим именем в доступном каталоге сохраняет
`NOT_FOUND`. Под root проверка идёт от имени `nobody`.
