...
CONTROLLED_CLOSED_WITH_CONTRACT=38
DISPOSED_CLOSED_ROWS=0
OPEN_INDEX_ROWS=311
CANONICAL_CONTROLS=48
```
...
`311` source rows остаются `OPEN`. Полный перечень и их source metadata находятся в `index/source-v4/SOURCE-INDEX.tsv`; этот документ не дублирует 349 строк вручную.
