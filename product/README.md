...
механизмы `config-line-with-runtime-v1`, `file-mode-owner-v1`, `optional-file-root-files-mode-v1`, `suid-sgid-applications-mode-v1`, `standard-system-paths-mode-v1`, `startup-files-write-protection-v1` и `kernel-cmdline-grub-v1`.
...
- `contracts/mechanism-file-mode-owner-v1.json`, `contracts/mechanism-optional-file-root-files-mode-v1.json`, `contracts/mechanism-suid-sgid-applications-mode-v1.json`, `contracts/mechanism-standard-system-paths-mode-v1.json` , `contracts/mechanism-startup-files-write-protection-v1.json` и `contracts/mechanism-kernel-cmdline-grub-v1.json` — `MECHANISM_AUTHORITY_V1` механизмов `file-mode-owner-v1` (SRC-0005), `optional-file-root-files-mode-v1` (SRC-0010), `suid-sgid-applications-mode-v1` (SRC-0013), `standard-system-paths-mode-v1` (SRC-0012) , `startup-files-write-protection-v1` (SRC-0009) и `kernel-cmdline-grub-v1` (G4, параметры загрузки ядра); содержат только поля, которые читает код;
...
Действующая APPLY authority — `mechanism-config-line-runtime-v1.json` r17, `mechanism-file-mode-owner-v1.json`, `mechanism-optional-file-root-files-mode-v1.json`, `mechanism-suid-sgid-applications-mode-v1.json`, `mechanism-standard-system-paths-mode-v1.json`, `mechanism-startup-files-write-protection-v1.json` и `mechanism-kernel-cmdline-grub-v1.json`. Реестры связывают `parameter_kind=sysctl` с механизмом `config-line-with-runtime-v1`, `parameter_kind=file-mode-owner` — с `file-mode-owner-v1`, `parameter_kind=optional-file-root-files-mode` — с `optional-file-root-files-mode-v1`, `parameter_kind=suid-sgid-applications` — с `suid-sgid-applications-mode-v1`, `parameter_kind=standard-system-paths-mode` — с `standard-system-paths-mode-v1`, `parameter_kind=startup-files-write-protection` — с `startup-files-write-protection-v1`, а `parameter_kind=kernel-cmdline` — с `kernel-cmdline-grub-v1`; scope вычисляется из корпуса. Прежняя SRC-0001 APPLY-цепочка сохранена только как historical bytes и из активных реестров/CLI удалена. Механизм sysctl прошёл отдельный implementation audit; полная продуктовая приёмка интеграции и последующий прогон семи поддерживаемых сред (Desktop — FIELD_COMPATIBILITY, отдельной строкой) выполняются отдельными gates. RESTORE не входит в целевую mutation-архитектуру. Policy noncompliance не равен execution
...
подменяется выводом generated CHECK. RESTORE не входит в продукт.

APPLY для kernel-cmdline выполняет механизм `kernel-cmdline-grub-v1` (решение человека 24.09.2026). Токен
добавляется в `GRUB_CMDLINE_LINUX` файла `/etc/default/grub.d/zz-securelinux-policy.cfg` (имя `zz-` —
чтобы файл читался после `init-select.cfg` и `kdump-tools.cfg` эталонных сред), затем выполняется
...
отсутствие) восстанавливается и `update-grub` запускается снова. Откат администратором — удалить
файл и выполнить `update-grub`.
...
APPLY реализован механизмами `config-line-with-runtime-v1`, `file-mode-owner-v1`, `optional-file-root-files-mode-v1`, `suid-sgid-applications-mode-v1`, `standard-system-paths-mode-v1`, `startup-files-write-protection-v1` и `kernel-cmdline-grub-v1`;
