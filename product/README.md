...
- `contracts/suid-sgid-applications-check-semantic-v2.json` — текущий read-only contract SRC-0013: SUID/SGID population на всех non-pseudo mounts, включая `nosuid`, проверка mode `go-w`; ветка `subset-of-file` (allowlist) в adapter/contract сохранена, но ни один control её не использует;
