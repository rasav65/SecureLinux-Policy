...
APPLY для 2.2.1 выполняет механизм `pam-wheel-su-v1` (решения человека 25.09.2026). `/etc/pam.d/su`
меняется, только если побайтно равен файлу пакета util-linux (SHA `fda16622…`, одинаков на семи
эталонных средах): закомментированная строка `# auth       required   pam_wheel.so` заменяется на
`auth       required   pam_wheel.so use_uid`. Нет группы `wheel` — `groupadd --system wheel` (GID
выбирает система), затем `gpasswd -a root wheel`; прочие участники существующей `wheel` не меняются,
`<user list>` источника остаётся решением администратора. После записи `su` доступен только
участникам `wheel`, поэтому запись выполняется, только если в группе `sudo` или `admin` есть
пользователь. Другой `/etc/pam.d/su` или пустые `sudo`/`admin` — исход `ABORTED_PRECONDITION_CONFLICT`
и блок «требуется решение администратора» с готовым действием (`ADMIN_ACTION_REQUIRED`). Порядок —
группа, затем PAM; при ошибке группа и файл возвращаются.
...
перекрыл бы основной файл. Каждый изменяемый файл готовится временным файлом в том же
каталоге и проверяется `sshd -t` до замены (основной — с текущими включаемыми, включаемый —
сам по себе); после замены — `sshd -t` всего дерева, `systemctl try-reload-or-restart
ssh.service` и итоговая проверка `sshd -T`. Ошибка любой из них — прежние байты всех
заменённых файлов возвращаются и сверяются, после попытки перезагрузки sshd
перезагружается повторно; несовпадение байтов — `FAILED_COMPENSATION`. Правила `Include`
(ссылки, тип префикса шаблона, имена с переводом строки) — как у CHECK. Блок «требуется
решение администратора» без записи: основной файл (всегда, до признания соответствия) или
изменяемый включаемый файл не обычный файл root без записи для группы и прочих (FIFO
...
По умолчанию вывод предназначен для человека: таблица с колонками `st`, `source`,
`control`, `current`, `required`, пунктир над заголовком, под ним и в конце. От 100
колонок терминала `source` шириной 29 символов («fstec-configuration-2026 §9.1» в одну
строку; решение пользователя 26.09.2026); длинные значения переносятся внутри своей
колонки. Уже 90 колонок — вертикальный вид «поле | значение».
