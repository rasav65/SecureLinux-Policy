...
def _render(control_id, roots, expected=EXPECTED_MASK):
    ...
    return "\n".join([
        ...
        '    if [[ -L "$_slp_root" ]]; then',
        emit + ' "ERROR" "cron-root:symlink" "ERROR"',
        "      return 0",
        "    fi",
        '    if [[ ! -e "$_slp_root" ]]; then',
        ...
        emit + ' "ERROR" "cron-root:ancestor-symlink" "ERROR"',
        "        return 0",
        "      fi",
        '      if [[ ! -d "$_slp_probe" ]]; then',
        emit + ' "ERROR" "cron-root:ancestor-invalid-type" "ERROR"',
        "        return 0",
        "      fi",
        '      if [[ ! -x "$_slp_probe" ]]; then',
        emit + ' "ERROR" "cron-root:ancestor-unsearchable" "ERROR"',
        "        return 0",
        "      fi",
        "      ((_slp_roots_absent+=1))",
        "      continue",
        "    fi",
        '    if [[ ! -d "$_slp_root" ]]; then',
        emit + ' "ERROR" "cron-root:invalid-type" "ERROR"',
        "      return 0",
        "    fi",
        ...
        emit + ' "ERROR" "cron-file:symlink" "ERROR"',
        "        return 0",
        "      fi",
        '      if [[ -d "$_slp_entry" ]]; then continue; fi',
        '      if [[ ! -f "$_slp_entry" ]]; then',
        emit + ' "ERROR" "cron-file:invalid-type" "ERROR"',
        "        return 0",
        "      fi",
        '      if ! _slp_mode=$(LC_ALL=C command /usr/bin/stat -c %a -- "$_slp_entry" 2>/dev/null); then',
        emit + ' "ERROR" "cron-file:mode-read-failed" "ERROR"',
        "        return 0",
        "      fi",
        '      if [[ ! $_slp_mode =~ ^[0-7]{3,4}$ ]]; then',
        emit + ' "ERROR" "cron-file:invalid-mode" "ERROR"',
        "        return 0",
        "      fi",
        "      ((_slp_checked+=1))",
