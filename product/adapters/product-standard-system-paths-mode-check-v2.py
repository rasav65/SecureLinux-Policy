...
def _render(control_id, exec_roots, lib_roots, module_root, expected=EXPECTED_MASK, dynamic_module=False, dynamic_root_path=False):
    ...
    lines += [
        ...
        emit + ' "ERROR" "root:resolve-failed" "ERROR"',
        '        return 0',
        '      fi',
        '      if [[ -z $_slp_resolved ]]; then',
        emit + ' "ERROR" "root:resolve-empty" "ERROR"',
        '        return 0',
        '      fi',
        '      if [[ ! -d "$_slp_resolved" ]]; then',
        emit + ' "ERROR" "root:invalid-type" "ERROR"',
        '        return 0',
        '      fi',
        '      if ! _slp_root_id=$(LC_ALL=C command /usr/bin/stat -Lc "%d:%i" -- "$_slp_resolved" 2>/dev/null); then',
        emit + ' "ERROR" "root:identity-failed" "ERROR"',
        '        return 0',
        '      fi',
        ...
        emit + ' "ERROR" "target:resolve-failed" "ERROR"',
        '            return 0',
        '          fi',
        '          if [[ -z $_slp_target ]]; then',
        emit + ' "ERROR" "target:resolve-empty" "ERROR"',
        '            return 0',
        '          fi',
        '          if [[ -L "$_slp_target" ]]; then',
        emit + ' "ERROR" "target:resolved-symlink" "ERROR"',
        '            return 0',
        '          fi',
        ...
        '  done',
        '  if (( _slp_exec == 0 )); then',
        emit + ' "ERROR" "population:missing-exec" "ERROR"',
        '    return 0',
        '  fi',
        '  if (( _slp_libraries == 0 )); then',
        emit + ' "ERROR" "population:missing-libraries" "ERROR"',
        '    return 0',
        '  fi',
        '  if (( _slp_modules == 0 )); then',
        emit + ' "ERROR" "population:missing-modules" "ERROR"',
        '    return 0',
        '  fi',
