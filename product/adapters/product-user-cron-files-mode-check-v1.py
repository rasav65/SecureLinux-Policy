#!/usr/bin/env python3
# Read-only adapter SRC-0011: рекурсивная проверка mode пользовательских cron-файлов.
import re

SEMANTIC_CONTRACT_ID = "user-cron-files-mode-check-semantic-v1"
ADAPTER_ID = "product-user-cron-files-mode-check-v1"
ADAPTER_CONTRACT_VERSION = "product-user-cron-files-mode-check-adapter-v1"
TARGET_ID = "ubuntu-24.04-x86_64"
PARAMETER_KIND = "user-cron-files-mode"
SUPPORTED_OPS = ("bits-clear",)
WIRE_RECORD_ID = "SLP-CHECK-V1"
CONTROL_ID_PATTERN = r"^(?!.*[\r\n])[A-Za-z0-9._-]+$"
CANONICAL_LOCATOR = "/var/spool/cron|/var/spool/cron/crontabs"
CANONICAL_ROOTS = ("/var/spool/cron", "/var/spool/cron/crontabs")
EXPECTED_MASK = "0022"
...
def _selftest():
    ...
    assert "/usr/bin/find -P" in src and "/usr/bin/sort -z" in src
