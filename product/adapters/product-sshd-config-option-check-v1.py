#!/usr/bin/env python3
"""Read-only source-faithful observer for one global sshd_config directive whose
source-required value is no (fstec-configuration-2026 SRC-0088, п.9.1)."""

import re

SEMANTIC_CONTRACT_ID = "sshd-config-option-check-semantic-v1"
ADAPTER_ID = "product-sshd-config-option-check-v1"
ADAPTER_CONTRACT_VERSION = "product-sshd-config-option-check-adapter-v1"
...
SUPPORTED_KEYS = ("PasswordAuthentication", "PermitEmptyPasswords", "PermitRootLogin")
EXPECTED_VALUE = "no"
...
def _shell_function_for_fixture(control_id, config_path, sshd_path, key, op, expected, sort_path="/usr/bin/sort"):
    ...
    if key not in SUPPORTED_KEYS or op != "eq" or expected != EXPECTED_VALUE:
    ...
    lines = [
        ...
        # `[[ ! -e ]]` истинно при любой ошибке stat; NOT_FOUND — только
        # доказанный ENOENT (`stat -c %F`, LC_ALL=C в команде), иначе ERROR.
        ...
        emit + ' "NOT_FOUND" "-" "FAIL"',
        "    else",
        emit + ' "ERROR" "sshd-binary:resolve-failed" "ERROR"',
        "    fi",
        "    return 0",
        "  fi",
        "  if [[ ! -f \"$_slp_sshd\" ]]; then",
        ...
        # (второй параметр — имя выходной переменной), который затем
        # разбирается; повторного открытия файла нет.
        ...
        "        continue",
        "      fi",
        "      [[ \"$_slp_pk\" == match || \"$_slp_pk\" == include || \"$_slp_pk\" == " + lkey + " ]] || continue",
        ...
        # Префикс шаблона не найден: пустой результат шаблона допустим только
        # при доказанном ENOENT префикса; иная ошибка stat и висячая ссылка — отказ.
        "            else",
        ...
        "            _slp_glob=(\"$_slp_px\")",
        # Включаемый файл без шаблона пропускается только при доказанном ENOENT.
        "          else",
        ...
        "    [[ \"${_slp_line,,}\" =~ ^" + lkey + "[[:space:]]+([^[:space:]]+)[[:space:]]*$ ]] || continue",
...
def _selftest():
    for key in SUPPORTED_KEYS:
        src = shell_function("CTRL", EXPECTED_LOCATOR, key, "eq", EXPECTED_VALUE)
        assert key not in src  # keys are matched case-insensitively at runtime
        assert key.lower() in src and "sshd" in src and "-T" in src and "-t" in src
        for other in SUPPORTED_KEYS:
            if other != key:
                assert other.lower() not in src, (key, other)
        for token in MUTATING_TOKENS:
            assert token not in src, token
    bad = (
        ("CTRL", "/tmp/sshd_config", "PermitRootLogin", "eq", "no"),
        ("CTRL", EXPECTED_LOCATOR, "permitrootlogin", "eq", "no"),
        ("CTRL", EXPECTED_LOCATOR, "UsePAM", "eq", "no"),
        ("CTRL", EXPECTED_LOCATOR, "PasswordAuthentication", "contains", "no"),
        ("CTRL", EXPECTED_LOCATOR, "PermitEmptyPasswords", "eq", "yes"),
    )
    for args in bad:
        try:
            shell_function(*args)
