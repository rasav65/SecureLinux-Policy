...
def _shell_function_for_fixture(control_id, config_path, sshd_path, key, op, expected, sort_path="/usr/bin/sort"):
    ...
    lines = [
        ...
        "    return 0",
        "  fi",
        "  [[ -x /usr/bin/readlink ]] || { " + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        "  [[ -x /usr/bin/find ]] || { " + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        "  [[ -x \"$_slp_sort\" ]] || { " + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        "  if [[ -L \"$_slp_sshd\" ]]; then",
        "    _slp_real=$(/usr/bin/readlink -f -- \"$_slp_sshd\" 2>/dev/null) || _slp_real=",
        "    [[ -n \"$_slp_real\" ]] || { " + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        "    _slp_sshd=$_slp_real",
        "  fi",
        "  if [[ ! -e \"$_slp_sshd\" ]]; then",
        emit + ' "NOT_FOUND" "-" "FAIL"',
