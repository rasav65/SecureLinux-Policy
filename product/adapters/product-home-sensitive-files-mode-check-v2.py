...
def _render(control_id, passwd_path, inventory_path):
    ...
    lines = [
        ...
        '    [[ ${_slp_inventory_names["$_slp_entry"]+x} ]] || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        '  done',
        ...
        '    (( ${#_slp_fields[@]} == 7 )) || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        '    _slp_name=${_slp_fields[0]}; _slp_uid=${_slp_fields[2]}; _slp_gid=${_slp_fields[3]}; _slp_home=${_slp_fields[5]}',
        '    [[ "$_slp_name" =~ ^[A-Za-z_][A-Za-z0-9_.-]*\\$?$ && "$_slp_uid" =~ ^[0-9]+$ && "$_slp_gid" =~ ^[0-9]+$ ]] || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        '    [[ -z ${_slp_seen_users["$_slp_name"]+x} ]] || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        '    _slp_seen_users["$_slp_name"]=1',
        '    [[ "$_slp_home" == /* ]] || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        '    ((_slp_accounts+=1))',
        ...
        '    )',
        '    (( ${#_slp_entries[@]} > 0 )) || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        '    _slp_i=$((${#_slp_entries[@]}-1)); _slp_marker=${_slp_entries[$_slp_i]}; unset \'_slp_entries[$_slp_i]\'',
        '    [[ "$_slp_marker" =~ ^__SLP_SCAN_RC=([0-9]+),([0-9]+)$ ]] || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        '    _slp_find_rc=${BASH_REMATCH[1]}; _slp_sort_rc=${BASH_REMATCH[2]}',
        '    (( _slp_find_rc == 0 && _slp_sort_rc == 0 )) || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        ...
        "        return 0",
        "      fi",
        '      [[ "$_slp_mode" =~ ^[0-7]{3,4}$ ]] || { ' + emit.strip() + ' "ERROR" "-" "ERROR"; return 0; }',
        '      ((_slp_checked+=1))',
