...
def shell_function(control_id, locator, key, op, expected):
    ...
    lines = [
        ...
        emit + ' "ERROR" "root:parent-not-found" "ERROR"',
        "      return 0",
        "    fi",
        '    if [[ ! -d "$_slp_parent" ]]; then',
        emit + ' "ERROR" "root:parent-invalid-type" "ERROR"',
        "      return 0",
        "    fi",
        '    if [[ ! -x "$_slp_parent" ]]; then',
        emit + ' "ERROR" "root:parent-unsearchable" "ERROR"',
        "      return 0",
        "    fi",
        '    if [[ ! -L "$_slp_path" && ! -e "$_slp_path" ]]; then',
        emit + ' "VALUE" "<absent>" "PASS"',
        "    else",
        emit + ' "ERROR" "root:state-changed" "ERROR"',
        "    fi",
        "    return 0",
        "  fi",
        '  if ! _slp_mode=$(LC_ALL=C command /usr/bin/stat -c %a -- "$_slp_path" 2>/dev/null); then',
        emit + ' "ERROR" "root:mode-read-failed" "ERROR"',
        "    return 0",
        "  fi",
        '  if [[ ! $_slp_mode =~ ^[0-7]{3,4}$ ]]; then',
        emit + ' "ERROR" "root:invalid-mode" "ERROR"',
        "    return 0",
        "  fi",
        "  ((_slp_checked+=1))",
