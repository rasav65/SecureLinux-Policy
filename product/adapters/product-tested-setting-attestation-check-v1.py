...
def _render(control_id, authority_path, source_id, expected_setting):
    ...
    return "\n".join([
        ...
        emit + ' "ERROR" "authority:header-read-failed" "ERROR"',
        "    return 0",
        "  fi",
        '  [[ "$_slp_header" == ' + _sh_single(AUTHORITY_HEADER) + ' ]] || { ' + emit.strip() + ' "ERROR" "authority:invalid-header" "ERROR"; return 0; }',
        "",
        '  while IFS= read -r _slp_line || [[ -n "$_slp_line" ]]; do',
        '    ((_slp_line_no+=1))',
        '    if (( _slp_line_no == 1 )); then [[ "$_slp_line" == "$_slp_header" ]] || { ' + emit.strip() + ' "ERROR" "authority:header-drift" "ERROR"; return 0; }; continue; fi',
