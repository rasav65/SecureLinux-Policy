...
### Added
- Добавлен CLI-флаг `--enable-corporate-password-policy`, допустимый только с `--apply`; он включает отдельную opt-in корпоративную парольную политику и имеет приоритет над `ENABLE_CORPORATE_PASSWORD_POLICY=0` из config-файла.
- Manifest дополнен полем `corporate_password_policy_enabled`, фиксирующим фактическое включение корпоративной парольной политики в текущем apply.
- Добавлен `tests/corporate-password-policy-cli-regression.sh`, проверяющий CLI parser, ограничения режима, приоритет над config и manifest-контракт.
- Добавлены `tests/sudo-policy-portability-regression.sh` и `tests/profile-aware-dry-run-regression.sh`; оба теста подключены к `tests/smoke.sh`.

- Добавлен `tools/write-sha256.py` для атомарного формирования переносимых GNU `sha256sum -c`-совместимых checksum-файлов с basename артефакта.
- Добавлен `tests/portable-sha256-regression.sh`, проверяющий отсутствие путей машины сборки, стандартную проверку `sha256sum -c` и отказ от неоднозначных/symlink-входов.
- Добавлен `tests/fstec-mapping-regression.sh`, сверяющий 40 статусов разделов 2.1–2.6 и итоговые счётчики с реестром `fstec_items`.
...
- Добавлен атомарный helper `atomic_write_command_output` для формирования конфигурации во временном файле с последующей заменой целевого файла только после успешного завершения генератора.
- Добавлен атомарный helper `remove_manifest_backup` для отката backup mapping, если подготовленная мутация не была зафиксирована.
- В README добавлена таблица всех 15 мер со статусом `partial`, реальной причиной ограничения и условием полного отката.
- Добавлен `tests/symlink-target-regression.sh`, проверяющий относительные, абсолютные и висячие ссылки, manifest restore, сохранение mode и отказ генератора.
- Добавлен `tests/manifest-bootstrap-regression.sh`, проверяющий обязательную успешную инициализацию manifest до запуска первого apply-модуля.
- Добавлен `tests/manifest-writer-contract-regression.sh`, проверяющий строгий контракт manifest writer-функций при отсутствующем, повреждённом и валидном manifest.
- Добавлен `tests/top-level-rc-regression.sh`, проверяющий агрегирование кодов возврата apply/restore-модулей и генераторов отчёта.
- Добавлен `tests/check-report-rc-regression.sh`, проверяющий RC preflight и генераторов check/report без превращения policy findings в ошибку выполнения.
- Добавлен `tests/typed-fstab-state-regression.sh`, проверяющий typed restore-state для `/etc/fstab`, отказ от `apply_report` как restore-базы и совместимость со старыми manifest.
- Добавлен `tests/shadow-state-regression.sh`, проверяющий typed empty-password state, отказ при symlink `/etc/shadow`, legacy fallback и отсутствие generic backup collision.
- Добавлен `tests/ufw-transaction-regression.sh`, проверяющий typed pre-state UFW/nftables, intent-маркеры, локальный rollback и восстановление после частичного сбоя enable.
- Добавлен `tests/state-dir-security-regression.sh`, проверяющий owner/mode, отказ от symlink и защищённый fallback `STATE_DIR`.
- Добавлен `tests/created-file-crash-regression.sh`, проверяющий двухфазный журнал `pending_created_files → created_files` и restore после аварийного завершения между записью manifest и созданием файла.
- Добавлен `tests/group-membership-crash-regression.sh`, проверяющий двухфазный журнал для создания `wheel` и добавления membership.
- Добавлен `tests/package-crash-regression.sh`, проверяющий package snapshot intent, атомарный commit фактической разницы и restore незавершённой установки.

- Добавлен `tests/service-state-crash-regression.sh`, проверяющий двухфазный журнал systemd pre-state, pending restore, отказ writer до мутации и portable validation имени unit.
- Добавлен `tests/manifest-resolution-regression.sh`, проверяющий явный manifest, активные manifest, архивный fallback и игнорирование symlink-кандидатов.
- Добавлен `tests/hardlink-safety-regression.sh`, проверяющий отказ backup, atomic write и restore для обычного файла с несколькими hardlink-именами.
- Добавлен `tests/extended-metadata-regression.sh`, проверяющий перенос user xattr, mode и, при доступности системных утилит/прав, POSIX ACL и file capabilities через атомарный managed-file update.
- Добавлен `tests/apport-suid-dumpable-regression.sh`, проверяющий Apport `ExecStartPost` drop-in, отсутствие отдельного unit/service-state, created-file/backup state и порядок restore.
- Добавлен `tests/write-once-sysctl-restore-regression.sh`, проверяющий Bash-обработку кодов runtime restore и реальные embedded-Python сценарии write-once, обычного и смешанного отказа.

### Fixed
- Исправлен порядок дополнительной меры Apport: `apply_apport_module` выполняется после `apply_sysctl_userspace_protection_module`, чтобы остановка Apport с изменением `fs.suid_dumpable=0` не искажала исходный userspace runtime snapshot; restore сохраняет реальное исходное значение, например `2` на Ubuntu 24.04.
- Исправлен `atomic_write_command_output`: producer теперь выполняется в текущем Bash-процессе до атомарной замены файла, поэтому shell-функции (`render_account_audit_report`, `render_kernel_module_blacklist`, auditd/fail2ban renderers) корректно работают наряду с внешними командами; сохранены preflight-проверки target, возврат RC producer и транзакционная семантика без замены target при ошибке.
- `--restore` больше не завершает штатный откат с ошибкой при подтверждённом невозможном понижении write-once sysctl: событие фиксируется как warning и `irreversible_changes`, а полный runtime baseline возвращается после reboot; обычные и смешанные ошибки по-прежнему дают ненулевой RC.
- Managed sudoers policy больше не содержит `Defaults logfile=...`, неподдерживаемый `sudo-rs`/его `visudo` в Ubuntu 26.04; обязательные `use_pty`, `timestamp_timeout`, `passwd_tries` и `secure_path` сохранены.
- `--apply --dry-run --enable-additional-measures` теперь сначала применяет профильные gates для AppArmor/AIDE (`strict+`) и fail2ban/rkhunter/mount hardening/`/tmp` tmpfs (`paranoid`), поэтому `baseline` и `strict` не объявляют запрещённые для профиля действия.
- На Ubuntu 24.04 устранена потеря `fs.suid_dumpable=0` после запуска Apport: базовый модуль 2.6 транзакционно создаёт `apport.service.d/60-securelinux-ng-suid-dumpable.conf` с `ExecStartPost`, не создаёт отдельный unit и не меняет enabled/active state; restore восстанавливает/удаляет drop-in до возврата runtime snapshot.
- Исправлен Apport-focused regression: документационный маркер проверяет полный путь `/etc/systemd/system/apport.service.d/60-securelinux-ng-suid-dumpable.conf`, а тест возвращает собственный ненулевой RC при статическом несоответствии вместо ложного успешного завершения после печати `RC_TEST`.
- `--check` теперь возвращает ненулевой RC при внутренних сбоях перечисления systemd unit targets и разбора `WHEEL_USERS`; обычные policy findings по-прежнему остаются `RISKY` и не считаются ошибкой выполнения.

- Все не restore-критические вызовы `record_manifest_modified_file` переведены на единый best-effort wrapper: отказ обновления информационного `modified_files` больше не игнорируется молча и фиксируется runtime warning; строгий отказ сохранён для транзакции `/etc/shadow`.
- Release checksum больше не содержит абсолютный путь локальной машины: `.sha256` формируется с переносимым basename.
- Синхронизирована `docs/fstec-mapping.md`: исправлены семь GRUB-статусов, пять дополнительных partial-мер и устаревшие счётчики `done=39`/`partial=20` на фактические `done=44`/`partial=15`.
...
- Все apply-пути, создающие backup или metadata snapshot, теперь проверяют результат записи backup mapping до начала мутации; при отказе manifest незарегистрированный backup или snapshot удаляется.
- Устранены ложные успешные результаты при ошибках snapshot, backup, manifest, chmod, chown и restore: модули сохраняют ненулевой итоговый RC и не объявляют объект исправленным после частичного сбоя.
- Транзакция обработки пустых полей пароля перестроена в порядок `подготовка state/shadow → manifest mapping → атомарная замена shadow`; при сбое mapping исходный shadow не изменяется, а при сбое commit mapping откатывается.
- Restore для пустых полей пароля по-прежнему не возвращает небезопасные пустые значения, но использует зарегистрированный в manifest список затронутых учётных записей.
- Усилены транзакционные apply/restore-потоки для runtime paths, cron paths, system paths, SUID/SGID, домашних каталогов, sudo paths, PAM wheel, GRUB, mount hardening, `/tmp` tmpfs, sysctl, faillock, password policy и coredump.
- Manifest и restore helpers теперь различают штатное отсутствие зарегистрированного состояния и ошибку чтения либо изменения manifest.
- Семь GRUB-мер переклассифицированы с устаревшего `policy-gated-detect-only` на фактический `grub-backup+reboot-required`.
- `pam_faillock`, corporate password policy, auditd, fail2ban и account audit переведены из `partial` в `done`, поскольку их контролируемый apply имеет полный автоматический restore.
- Для `/tmp` tmpfs и mount hardening тип restore уточнён как `managed-file+reboot-required`; итоговые счётчики исправлены на `implemented=59`, `done=44`, `partial=15`.
- Документация удаления пакетов синхронизирована с реализованным restore пакетов password policy, auditd и fail2ban.
- Устранена потеря данных при работе с символьными ссылками: `atomic_write_command_output()` больше не разыменовывает managed-путь, поэтому файл-цель ссылки не изменяется, а restore возвращает исходную ссылку из `cp -a` backup.
- Исправлено наследование ослабленных metadata через symlink-target: атомарная запись использует `lstat()` управляемого pathname, наследует mode/uid/gid/xattrs только от обычного файла, а для ссылки применяет `default_mode` и не читает xattrs её цели.
- GRUB переведён на общие symlink-safe `backup_file_checked()` и `restore_file_from_manifest()`; прямые `cp` удалены.
- Три managed-файла coredump переведены с прямого `printf >` на атомарную запись; backup и определение существования теперь учитывают рабочие и висячие ссылки.
- `run_apply_mode()` теперь немедленно завершает apply с RC=1, если `manifest_init()` не смог создать restore-состояние; hardening-модули после отказа bootstrap не запускаются.
- Manifest writer-функции больше не возвращают ложный успех при пустом или отсутствующем `MANIFEST_FILE`; исключение оставлено только для явного `DRY_RUN=1`.
- `run_apply_mode()` и `run_restore_mode()` теперь учитывают RC каждого модуля и report-шагов, продолжают независимые операции и возвращают общий RC=1 при любом локальном сбое.
- `run_check_mode()` и `run_report_mode()` теперь учитывают ошибки preflight, STATE_DIR и генераторов отчёта; `run_preflight()` имеет явный успешный RC=0, а несоответствия check-модулей по-прежнему отражаются как policy findings, а не как сбой исполнения.
- Restore `mount_hardening` и `/tmp` больше не зависит от строк `apply_report`: до изменения `/etc/fstab` записывается typed `module_state.<module>.restore_required`, а общий backup восстанавливается не более одного раза.
- Список УЗ с пустым password field вынесен из generic `backups` в `module_state.empty_passwords`; apply безопасно отклоняет symlink `/etc/shadow`, а restore использует политику `security-preserving-nonrestore`.
- UFW/nftables переведены с restore-критичных строк `apply_report` на typed `module_state.firewall`; partial failure после runtime enable возвращает RC=1, выполняет локальный disable и сохраняет marker для последующего restore.
- `STATE_DIR` больше не принимает symlink, не-каталог или каталог чужого владельца; mode `0700` устанавливается через открытый descriptor и проверяется, а apply/restore прекращаются до мутаций при отказе security validation.
- Все 18 транзакций создания managed-файлов регистрируют путь в `pending_created_files` до первой мутации и переводят его в `created_files` после успешной проверки; restore учитывает оба списка и закрывает crash-window `create → manifest`.
- Создание группы `wheel` и добавление пользователей переведены на durable `pending_created_groups`/`pending_group_memberships` до `groupadd`/`gpasswd -a` с атомарным commit и restore обоих состояний.
- Все package-install пути переведены на `pending_package_transactions → apt-get install → installed_packages`; частично успешная установка фиксируется по фактической разнице, а restore обрабатывает pending snapshot без незарегистрированного package-window.

- Network sysctl unit, AppArmor, rsyslog, chrony, unattended-upgrades и apport переведены на `pending_service_transactions → systemctl → service_transactions`; restore автоматически возвращает исходные enabled/active состояния после штатного или аварийного apply.
- Автоматический выбор manifest теперь использует последний `*.json.bak-YYYYMMDD-HHMMSS`, если активные manifest отсутствуют после архивирования перед повторным apply; symlink-кандидаты исключены.
- В Mermaid-схеме удалена неверная прямая стрелка из apply-manifest в модульный restore; показано, что manifest используется отдельным последующим запуском `--restore`.
- Managed-файлы с `st_nlink > 1` теперь отклоняются до backup, atomic replace и restore removal, поскольку manifest не хранит hardlink topology и не может безопасно восстановить связь inode.
- `atomic_write_command_output()` теперь переносит и проверяет все читаемые xattrs существующего managed-файла до `os.replace`; POSIX ACL, file capabilities и security labels больше не теряются при создании нового hardened inode, а ошибка metadata-copy оставляет исходный путь неизменным.
- `atomic_write_command_output()` теперь различает отсутствие поддержки xattrs (`ENOTSUP`/`EOPNOTSUPP`/`ENOSYS`) и реальные ошибки доступа/чтения: неподдерживаемая ФС продолжает запись без xattrs, а неожиданные ошибки сохраняют fail-before-replace.
- Удалены 12 неиспользуемых верхнеуровневых констант и лишний `import re` из package pending writer; broad cleanup-exception с обязательным `raise` сохранён намеренно.

### Tested
- Регрессии `tests/apport-suid-dumpable-regression.sh`, `tests/additional-measures-optin-regression.sh` и `tests/top-level-rc-regression.sh` подтвердили порядок userspace snapshot → дополнительное отключение Apport, сохранение opt-in gate и корректную агрегацию RC.
- Добавлен `tests/atomic-writer-shell-function-regression.sh`: проверены Bash-function producer с глобальными переменными/массивами и helper-функциями, внешний producer, heredoc, сохранение ошибочного RC, preflight до producer и cleanup временных файлов; повторно подтверждены `symlink-target-regression.sh` и `extended-metadata-regression.sh`.
- Corporate-password-policy CLI regression подтвердил новый `--apply`-only флаг, отказ с `--check`/`--restore`/`--report`, CLI-приоритет над config и наличие `corporate_password_policy_enabled` в manifest-контракте.
- Focused regression подтвердил: только write-once отказ даёт `RC=0`, warning и `irreversible_changes`; обычный отказ известного ключа, смешанный отказ и отсутствующий `/proc/sys` дают `RC=1`; уже установленное целевое значение даёт `RC=0` без предупреждений.
- На Ubuntu 26.04 Minimal с `sudo-rs 0.2.13` изолированно подтверждено: `Defaults logfile=...` отклоняется, остальные managed sudoers directives проходят `visudo`; исходный baseline-кандидат дошёл до единственной ошибки sudo policy.
- Статический profile-aware regression проверяет порядок profile gate перед `DRY_RUN` во всех шести полностью профильных модулях.
- VM-тест Ubuntu Server 24.04.4 воспроизвёл цепочку `systemd-sysctl: fs.suid_dumpable=0 → apport --start: fs.suid_dumpable=2`; focused regression фиксирует `ExecStartPost` drop-in, apply/restore order, отсутствие отдельного unit и регистрацию в smoke-наборе.
- Негативная focused-проверка подтвердила, что отсутствие обязательного маркера Apport drop-in теперь приводит к `RC_TEST=1` и ненулевому коду завершения всего regression-скрипта.
- Check/report RC regression подтверждает, что сбои `systemd_unit_candidates()` и `configured_wheel_users()` добавляют execution error и приводят к ненулевому RC, не превращая обычные `RISKY`-результаты в ошибки.

- Manifest-writer regression проверяет, что 37 информационных call sites используют best-effort wrapper, ошибка writer превращается в предупреждение, валидная запись обновляет `modified_files`, а restore-критическая транзакция `/etc/shadow` сохраняет строгий RC.
- Portable-SHA256 regression подтверждает проверку checksum после переноса, атомарную запись и неизменность symlink-target при отказе output path.
- Добавлены постоянные regression-проверки порядка `backup/snapshot → manifest → mutation`, удаления незарегистрированных backup и запрета мутации после ошибки mapping.
- Добавлены постоянные regression-проверки транзакции empty-password, отката mapping после ошибки commit и безопасного поведения restore.
- Focused failure-injection тестами проверены успешные операции, ошибки snapshot/backup, ошибки manifest, ошибки финального commit и ошибки rollback.
- Подтверждено, что все 38 вызовов `record_manifest_backup` обрабатывают RC, а единственный вызов `remove_manifest_backup` также защищён проверкой результата.
- Выполнены профильные regression-тесты, `tests/architecture-regression.sh`, полный `tests/smoke.sh`, проверки синтаксиса Bash и `git diff --check`.
- Manifest-resolution regression проверяет приоритет явного пути, `manifest-*.json`, `manifest.json`, архивного fallback и отказ от symlink-кандидатов.
- Добавлены постоянные проверки классификации GRUB, полного списка оставшихся `partial`, счётчиков `44/15` и согласованности таблицы ограничений restore с исходным реестром.
- Symlink regression подтверждает, что цели относительных и абсолютных ссылок не меняются, висячая ссылка не создаёт свою цель, manifest restore возвращает исходный link target, а ошибка генератора не повреждает обычный файл.
- Дополнительные symlink-metadata сценарии подтверждают, что ссылки на файл mode `0666`, каталог mode `0755` и `/dev/null` заменяются managed-файлом строго с `default_mode`, без наследования прав target.
- Extended-metadata regression дополнен injected-сценариями: `ENOTSUP` разрешает атомарную запись с сохранением mode, а `EACCES` возвращает ненулевой RC и оставляет исходный файл неизменным.
- Failure-injection regression подтверждает, что ошибка `manifest_init()` вызывает один `add_error`, не допускает ни одного post-bootstrap вызова и возвращает общий RC=1.
- Manifest-writer regression подтверждает RC=1 без manifest и при повреждённом JSON, RC=0 в dry-run и корректную запись всех шести полей в валидный manifest.
- Top-level RC regression подтверждает продолжение последующих restore/apply-шагов после локальной ошибки, отсутствие дублирования module-specific error и ненулевой итоговый RC при сбое `write_report`.
- Check/report RC regression подтверждает ненулевой RC при отказе preflight/report writer, продолжение последующих шагов, отсутствие дублирования ошибок и сохранение RC=0 для обычных risky-результатов проверки.
- Typed-fstab regression подтверждает восстановление исходного `/etc/fstab` при пустом `apply_report`, пропуск модулей без typed-маркера, однократный restore общего backup и fallback для legacy manifest.
- Shadow-state regression подтверждает неизменность symlink и его target, отсутствие мутации при отказе typed writer, корректный typed restore и совместимость с legacy manifest.
- UFW transaction regression подтверждает блокировку мутации при отказе writer, локальный rollback после сбоя `systemctl enable`, restore оставшегося активным UFW, точный возврат active/enabled nftables и отказ на повреждённом typed state.
- State-dir regression подтверждает безопасное создание и исправление mode, отказ от symlink/не-каталога/чужого owner, защищённый fallback check/report и fail-fast apply.
- Created-file crash regression подтверждает pending-маркер до мутации во всех 19 файловых транзакциях, распознавание pending-состояния restore helpers, атомарный commit и совместимость прямой записи legacy `created_files`.
- Group-membership crash regression подтверждает pending-маркеры до `groupadd`/`gpasswd -a`, commit в штатном потоке, restore после crash до и после мутации и отказ на повреждённом journal.
- Package crash regression подтверждает единственный централизованный `apt-get install`, порядок `pending → install → commit`, фиксацию partial install, блокировку install при отказе pending writer, auto-purge pending-разницы и отказ на повреждённом snapshot.

- Service-state regression подтверждает атомарный commit, восстановление pending-состояния, блокировку systemctl при отказе writer и отклонение имени unit с лишним обратным слешем.
- Hardlink-safety regression подтверждает неизменность обоих имён inode при отказе backup/atomic/restore, отказ от hardlinked backup и штатную обработку обычного файла с одним именем.
- Extended-metadata regression подтверждает порядок `fchown/fchmod → setxattr → verify → fsync → replace`, сохранение user xattr и условную проверку ACL/capabilities на поддерживающей среде.
...
- Focused-проверкой подтверждена точная синхронизация `docs/fstec-mapping.md` с source registry: 40 официальных пунктов (`done=30`, `partial=10`) и общий реестр (`done=44`, `partial=15`).
